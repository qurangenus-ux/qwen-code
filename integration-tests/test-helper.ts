...
export class TestRig {
  ...
  getDefaultTimeout() {
    ...
    if (env['DOCT_SANDBOX']) return 30000; // 30s in containers
  ...
  setup(
    ...
    mkdirSync(doctDir, { recursive: true });
    ...
    const settings = {
      ...
      sandbox: env.DOCT_SANDBOX !== 'false' ? env.DOCT_SANDBOX : false,
  ...
  /**
   * The command and args to use to invoke Doct Code CLI. Allows us to switch
   * between using the bundled gemini.js (the default) and using the installed
   * 'doct' (used to verify npm bundles).
   */
  private _getCommandAndArgs(extraInitialArgs: string[] = []): {
    ...
    const command = isNpmReleaseTest ? 'doct' : 'node';
    ...
    return this.poll(
      () => {
        ...
        return logs.some(
          (logData) =>
            logData.attributes &&
            logData.attributes['event.name'] === `doct-code.${eventName}`,
        );
      },
      timeout,
      100,
          ...
          if (braceDepth === 0) {
            ...
            try {
              ...
              if (
                ...
                obj.attributes &&
                obj.attributes['event.name'] === 'doct-code.tool_call'
              ) {
                logs.push({
  ...
  readToolLogs() {
    ...
    for (const logData of parsedLogs) {
      ...
      if (
        logData.attributes &&
        logData.attributes['event.name'] === 'doct-code.tool_call'
      ) {
  ...
  readLastApiRequest(): Record<string, unknown> | null {
    ...
    const apiRequests = logs.filter(
      (logData) =>
        logData.attributes &&
        logData.attributes['event.name'] === 'doct-code.api_request',
    );
      ...
      if (logData.scopeMetrics) {
        for (const scopeMetric of logData.scopeMetrics) {
          for (const metric of scopeMetric.metrics) {
            if (metric.descriptor.name === `doct-code.${metricName}`) {
              return metric;
            }
          }
        }
