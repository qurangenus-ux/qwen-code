#!/usr/bin/env python3
"""
Doct Concurrent Runner - Execute multiple CLI tasks across different models concurrently.

This tool creates isolated git worktrees for each task/model combination and executes
the Doct CLI in parallel with status tracking and output capture.
"""
...
class RunConfig:
    ...
    worktree_base: Path = field(default_factory=lambda: Path.home() / ".doct" / "worktrees")
...
class RunRecord:
    ...
    model: str
    status: RunStatus
    auth_type: Optional[str] = None  # e.g. "anthropic" for doct --auth-type
...
class GitWorktreeManager:
    ...
    async def collect_session_log(self, worktree_dir: Path, output_dir: Path) -> Optional[Tuple[Path, str, Path]]:
        ...
        ~/.doct/projects/{projectId}/chats/{sessionId}.jsonl
        ...
        doct_dir = Path.home() / ".doct"
        chats_dir = doct_dir / "projects" / project_id / "chats"
...
<head>
    <meta charset="UTF-8">
    <title>Doct Runner Report</title>
    <style>
...
<body>
    <h1>Doct Runner Execution Report</h1>
    <div class="summary">
...
class DoctRunner:
    """Executes the Doct CLI for a specific task and model."""
    ...
    async def run(
        ...
        """Execute the Doct CLI for each prompt sequentially."""
        ...
        for prompt_index, prompt_text in enumerate(task.prompts, start=1):
            ...
            env["DOCT_CODE_ROOT"] = str(worktree_dir_resolved)
    ...
    def _build_command(self, run: RunRecord, prompt_text: str, use_continue: bool = False) -> List[str]:
        """Build the doct CLI command for a single prompt."""
        cmd = ["doct"]
...
def load_config(config_path: Path) -> RunConfig:
    ...
    return RunConfig(
        ...
        worktree_base=Path(data.get("worktree_base", "~/.doct/worktrees")).expanduser(),
...
async def execute_single_run(
    ...
    try:
        ...
        await doct_runner.run(run, worktree_dir, output_dir)
...
async def run_all(config: RunConfig, console: Console) -> ExecutionState:
    ...
    doct_runner = DoctRunner(config, console)
    ...
    async def execute_with_limit(run: RunRecord):
        async with semaphore:
            await execute_single_run(
                run, config, tracker, worktree_manager, doct_runner, console
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Doct Concurrent Runner - Execute multiple CLI tasks across models"
    )
    parser.add_argument(
        "config",
        type=Path,
