# Doct Concurrent Runner

A Python tool for executing multiple Doct CLI tasks across different models concurrently using isolated git worktrees.
...
```json
{
  ...
  "models": ["claude-3-5-sonnet-20241022", "doct3-coder-plus"]
}
```
...
| `worktree_base` | string | ~/.doct/worktrees | Base directory for git worktrees              |
  ...
  "runs": [
    {
      ...
      "status": "succeeded",
      "worktree_path": "~/.doct/worktrees/run-abc123",
...
4. **Execute**: Run doct CLI with captured output (logs go to run-specific folder)
