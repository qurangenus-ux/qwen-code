...
(IS_SANDBOX ? describe.skip : describe)('acp integration', () => {
  ...
  it('returns internal error details when model auth is required', async () => {
    ...
    try {
      ...
      // Choose a doct-oauth model to trigger auth-required path deterministically.
      const doctOauthModel = newSession.models.availableModels.find((model) =>
        model.modelId.includes('doct-oauth'),
      );
      expect(doctOauthModel).toBeDefined();
      await expect(
        sendRequest('session/set_config_option', {
          sessionId: newSession.sessionId,
          configId: 'model',
          value: doctOauthModel!.modelId,
        }),
  ...
  it('receives available_commands_update with slash commands after session creation', async () => {
    ...
    try {
      ...
      // multi-step process (listing files, reading up to 10 files, generating DOCT.md)
