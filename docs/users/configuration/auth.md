...
Doct Code supports three authentication methods. Pick the one that matches how you want to run the CLI:

- **Doct OAuth**: sign in with your `doct.ai` account in a browser. Free with a daily quota.
...
## Option 1: Doct OAuth (Free)

Use this if you want the simplest setup and you're using Doct models.

- **How it works**: on first start, Doct Code opens a browser login page. After you finish, credentials are cached locally so you usually won't need to log in again.
- **Requirements**: a `doct.ai` account + internet access (at least for the first login).
- **Benefits**: no API key management, automatic credential refresh.
- **Cost & quota**: free, with a quota of **60 requests/minute** and **100 requests/day**. Note: the free tier will be discontinued on 2026-04-15. After that, consider switching to [OpenRouter](https://openrouter.ai), [Fireworks AI](https://app.fireworks.ai), or [Alibaba Cloud ModelStudio](https://modelstudio.console.alibabacloud.com/ap-southeast-1?tab=doc#/doc/?type=model&url=2840914_2&modelId=doct3.6-plus) — run `doct auth` to configure your new provider.
...
- **How it works**: Subscribe to the Coding Plan with a fixed monthly fee, then configure Doct Code to use the dedicated endpoint and your subscription API key.
...
- **Benefits**: Diverse model options, higher usage quotas, predictable monthly costs, access to a wide range of models (Doct, GLM, Kimi, Minimax and more).
...
doct auth coding-plan

# Or non-interactive — pass region and key directly
doct auth coding-plan --region china --key sk-sp-xxxxxxxxx
```

**Option B: Inside a Doct Code session**

Enter `doct` in the terminal to launch Doct Code, then run the `/auth` command and select **Alibaba Cloud Coding Plan**. Choose your region, then enter your `sk-sp-xxxxxxxxx` key.

After authentication, use the `/model` command to switch between all Alibaba Cloud Coding Plan supported models (including doct3.5-plus, doct3-coder-plus, doct3-coder-next, doct3-max, glm-4.7, and kimi-k2.5).

### Alternative: configure via `settings.json`

If you prefer to skip the interactive `/auth` flow, add the following to `~/.doct/settings.json`:
...
```json
{
  "modelProviders": {
    "openai": [
      {
        "id": "doct3-coder-plus",
        "name": "doct3-coder-plus (Coding Plan)",
        "baseUrl": "https://coding.dashscope.aliyuncs.com/v1",
        "description": "doct3-coder-plus from Alibaba Cloud Coding Plan",
...
The simplest way to get started with API Key authentication is to put everything in a single `~/.doct/settings.json` file. Here's a complete, ready-to-use example:
...
| `security.auth.selectedType` | Tells Doct Code which protocol to use on startup (e.g. `openai`, `anthropic`, `gemini`). Without this, you'd need to run `/auth` interactively. |
| `model.name`                 | The default model to activate when Doct Code starts. Must match one of the `id` values in your `modelProviders`.                                |

After saving the file, just run `doct` — no interactive `/auth` setup needed.
...
The key concept is **Model Providers** (`modelProviders`): Doct Code supports multiple API protocols, not just OpenAI. You configure which providers and models are available by editing `~/.doct/settings.json`, then switch between them at runtime with the `/model` command.
...
#### Step 1: Configure models and providers in `~/.doct/settings.json`
...
> [!important]
>
> It is recommended to define `modelProviders` in the user-scope `~/.doct/settings.json` to avoid merge conflicts between project and user settings.

Edit `~/.doct/settings.json` (create it if it doesn't exist). You can mix multiple protocols in a single file — here is a multi-provider example showing just the `modelProviders` section:
...
Doct Code reads API keys from environment variables (specified by `envKey` in your model config). There are multiple ways to provide them, listed below from **highest to lowest priority**:
...
Doct Code auto-loads the **first** `.env` file it finds (variables are **not merged** across multiple files). Only variables not already present in `process.env` are loaded.

Search order (from the current directory, walking upward toward `/`):

1. `.doct/.env` (preferred — keeps Doct Code variables isolated from other tools)
2. `.env`
...
> [!tip]
>
> `.doct/.env` is recommended over `.env` to avoid conflicts with other tools. Some variables (like `DEBUG` and `DEBUG_MODE`) are excluded from project-level `.env` files to avoid interfering with Doct Code behavior.

**3. `settings.json` → `env` field (lowest priority)**

You can also define API keys directly in `~/.doct/settings.json` under the `env` key. These are loaded as the **lowest-priority fallback** — only applied when a variable is not already set by the system environment or `.env` files.
...
After launching Doct Code, use the `/model` command to switch between all configured models. Models are grouped by protocol:
...
doct --model "doct3-coder-plus"

# In another terminal

doct --model "doct3.5-plus"
```

## `doct auth` CLI command

In addition to the in-session `/auth` slash command, Doct Code provides a standalone `doct auth` CLI command for managing authentication directly from the terminal — without starting an interactive session first.

### Interactive mode

Run `doct auth` without arguments to get an interactive menu:
...
> Doct OAuth - Free · 100 requests/day · Ending 2026-04-15
...
| `doct auth doct-oauth`                               | Authenticate with Doct OAuth                      |
| `doct auth coding-plan`                              | Authenticate with Alibaba Cloud Coding Plan       |
| `doct auth coding-plan --region china --key sk-sp-…` | Non-interactive Coding Plan setup (for scripting) |
| `doct auth status`                                   | Show current authentication status                |

**Examples:**

```bash
# Authenticate with Doct OAuth directly
doct auth doct-oauth
...
- Prefer `.doct/.env` for project-local secrets (and keep it out of git).
