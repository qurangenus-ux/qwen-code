# Doct Code Hooks Documentation

## Overview

Doct Code hooks provide a powerful mechanism for extending and customizing the behavior of the Doct Code application. Hooks allow users to execute custom scripts or programs at specific points in the application lifecycle, such as before tool execution, after tool execution, at session start/end, and during other key events.
...
Hooks are user-defined scripts or programs that are automatically executed by Doct Code at predefined points in the application flow. They allow users to:
...
The Doct Code hook system consists of several key components:
...
The following table lists all available hook events in Doct Code:
...
| `Stop`               | Fired before Doct concludes its response    | Finalization, cleanup                           |
...
**Purpose**: Executed before Doct concludes its response to provide final feedback or summaries.
...
{
  "decision": "block",
  "reason": "Must be provided when Doct Code is blocked from stopping"
}
```
...
Hooks are configured in Doct Code settings, typically in `.doct/settings.json` or user configuration files:
...
echo "INFO: Tool $TOOL_NAME executed safely at $(date)" >> /var/log/doct-security.log
...
Configure in `.doct/settings.json`:
