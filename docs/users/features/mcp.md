# Connect Doct Code to tools via MCP

Doct Code can connect to external tools and data sources through the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/introduction). MCP servers give Doct Code access to your tools, databases, and APIs.

## What you can do with MCP

With MCP servers connected, you can ask Doct Code to:
...
Doct Code loads MCP servers from `mcpServers` in your `settings.json`. You can configure servers either:

- By editing `settings.json` directly
- By using `doct mcp` commands (see [CLI reference](#doct-mcp-cli))
...
```bash
doct mcp add --transport http my-server http://localhost:3000/mcp
```
...
3. Restart Doct Code in the same project (or start it if it wasn’t running yet), then ask the model to use tools from that server.

## Where configuration is stored (scopes)

Most users only need these two scopes:

- **Project scope (default)**: `.doct/settings.json` in your project root
- **User scope**: `~/.doct/settings.json` across all projects on your machine

Write to user scope:

```bash
doct mcp add --scope user --transport http my-server http://localhost:3000/mcp
```
...
```bash
doct mcp add pythonTools -e DATABASE_URL=$DB_CONNECTION_STRING -e API_KEY=$EXTERNAL_API_KEY \
...
```bash
doct mcp add --transport http httpServerWithAuth http://localhost:3000/mcp \
...
```bash
doct mcp add --transport sse sseServer http://localhost:8080/sse --timeout 30000
```
...
Use `includeTools` / `excludeTools` to restrict tools exposed by a server (from Doct Code’s perspective).
...
- **Server shows “Disconnected” in `doct mcp list`**: verify the URL/command is correct, then increase `timeout`.
- **Stdio server fails to start**: use an absolute `command` path, and double-check `cwd`/`env`.
- **Environment variables in JSON don’t resolve**: ensure they exist in the environment where Doct Code runs (shell vs GUI app environments can differ).
...
### Manage MCP servers with `doct mcp`
