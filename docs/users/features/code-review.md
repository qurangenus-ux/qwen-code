...
When reviewing a PR, `/review` creates a temporary git worktree (`.doct/tmp/review-pr-<number>`) instead of switching your current branch. This means:
...
- Model attribution footer on each comment (e.g., _— doct3-coder via Doct Code /review_)
...
/review 123
# → "Previous review used doct3-coder. Running full review with gpt-4o for a second opinion."
```

Cache is stored in `.doct/review-cache/` and tracks both the commit SHA and model ID. Make sure this directory is in your `.gitignore` (a broader rule like `.doct/*` also works). If the cached commit was rebased away, it falls back to a full review.

## Review Reports

For same-repo reviews, results are saved as a Markdown file in your project's `.doct/reviews/` directory (cross-repo lightweight reviews skip report persistence):

```
.doct/reviews/2026-04-06-143022-pr-123.md
.doct/reviews/2026-04-06-150510-local.md
```
