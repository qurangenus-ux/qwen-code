...
> Create, manage, and share Skills to extend Doct Code's capabilities.

This guide shows you how to create, use, and manage Agent Skills in **Doct Code**. Skills are modular capabilities that extend the model's effectiveness through organized folders containing instructions (and optionally scripts/resources).

## Prerequisites

- Doct Code (recent version)
- Basic familiarity with Doct Code ([Quickstart](../quickstart.md))
...
- Extend Doct Code for your workflows
...
Personal Skills are available across all your projects. Store them in `~/.doct/skills/`:

```bash
mkdir -p ~/.doct/skills/my-skill-name
```
...
Project Skills are shared with your team. Store them in `.doct/skills/` within your project:
...
## Instructions
Provide clear, step-by-step guidance for Doct Code.
...
Doct Code currently validates that:
...
Doct Code discovers Skills from:

- Personal Skills: `~/.doct/skills/`
- Project Skills: `.doct/skills/`
...
To see which extensions provide skills, check the extension's `doct-extension.json` file for a `skills` field.

To view available Skills, ask Doct Code directly:
...
cat ~/.doct/skills/my-skill/SKILL.md
```
...
If Doct Code doesn't use your Skill, check these common issues:
...
- Personal Skills: `~/.doct/skills/<skill-name>/SKILL.md`
- Project Skills: `.doct/skills/<skill-name>/SKILL.md`

```bash
# Personal
ls ~/.doct/skills/my-skill/SKILL.md
...
Run Doct Code with debug mode to see Skill loading errors:
...
1. Add the Skill under `.doct/skills/`
2. Commit and push
...
```bash
# Personal Skill
code ~/.doct/skills/my-skill/SKILL.md

# Project Skill
code .doct/skills/my-skill/SKILL.md
```

Changes take effect the next time you start Doct Code. If Doct Code is already running, restart it to load the updates.
