...
Doct Code includes a contextual tips system that helps you discover features and stay aware of session state.

## Startup Tips

Each time you launch Doct Code, a tip is shown in the header area. Tips are selected by priority first, then rotated across sessions using LRU (least-recently-used) scheduling among tips of the same priority, so you see a different tip each time.
...
| < 10     | `DOCT.md` project context, `--continue` / `--resume` |
...
During a conversation, Doct Code monitors your context window usage and shows tips when action may be needed:
...
Tip display history is persisted at `~/.doct/tip_history.json`. This file tracks:
...
To hide all tips (both startup and post-response), set `ui.hideTips` to `true` in `~/.doct/settings.json`:
