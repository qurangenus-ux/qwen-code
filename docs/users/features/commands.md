...
This document details all commands supported by Doct Code, helping you efficiently manage sessions, customize the interface, and control its behavior.

Doct Code commands are triggered through specific prefixes and fall into three categories:

| Prefix Type                | Function Description                                | Typical Use Case                                                 |
| -------------------------- | --------------------------------------------------- | ---------------------------------------------------------------- |
| Slash Commands (`/`)       | Meta-level control of Doct Code itself              | Managing sessions, modifying settings, getting help              |
...
Slash commands are used to manage Doct Code sessions, interface, and basic behavior.
...
| `/model --fast`  | Set a lighter model for prompt suggestions    | `/model --fast doct3-coder-flash`             |
...
| `/qc-helper` | Answer questions about Doct Code usage and configuration            | `/qc-helper how do I configure MCP?`              |
...
| `/bug`      | Submit issue about Doct Code                    | `/bug Button click unresponsive` |
...
In addition to the in-session `/auth` slash command, Doct Code provides standalone CLI subcommands for managing authentication directly from the terminal:
...
> [!tip]
>
> These commands run outside of a Doct Code session. Use them to configure authentication before starting a session, or in scripts and CI environments. See the [Authentication](../configuration/auth) page for full details.
...
Exclamation commands allow you to execute system commands directly within Doct Code.
...
Environment Variables: Commands executed via `!` will set the `DOCT_CODE=1` environment variable.
...
| Global Commands  | `~/.doct/commands/`                        | Available in all projects              | Low      | Personal frequently used commands, cross-project use |
| Project Commands | `<project root directory>/.doct/commands/` | Project-specific, version-controllable | High     | Team sharing, project-specific commands              |
...
| ---------------------------------------- | ----------------- | --------------------- |
| `~/.doct/commands/test.md`               | `/test`           | `/test Parameter`     |
| `<project>/.doct/commands/git/commit.md` | `/git:commit`     | `/git:commit Message` |
...
| ----------------------------- | ----------------------------------------- |
| 1. Create directory structure | `mkdir -p ~/.doct/commands/refactor`      |
| 2. Create command file        | `touch ~/.doct/commands/refactor/pure.md` |
