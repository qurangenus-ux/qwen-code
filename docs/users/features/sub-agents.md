...
Subagents are specialized AI assistants that handle specific types of tasks within Doct Code. They allow you to delegate focused work to AI agents that are configured with task-specific prompts, tools, and behaviors.
...
In addition to named subagents, Doct Code supports **implicit forking** — when the AI omits the `subagent_type` parameter, it triggers a fork that inherits the parent's full conversation context.
...
- **Project-level**: `.doct/agents/` (highest precedence)
- **User-level**: `~/.doct/agents/` (fallback)
...
To see which extensions provide subagents, check the extension's `doct-extension.json` file for an `agents` field.
...
Doct Code proactively delegates tasks based on:
