...
This guide covers setting up a Doct Code channel on Telegram.
...
doct channel start my-telegram
...
**Photos:** Send a photo and the agent will analyze it using its vision capabilities. This requires a multimodal model — add `"model": "doct3.5-plus"` (or another vision-capable model) to your channel config. Photo captions are passed as the message text.
