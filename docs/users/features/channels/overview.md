...
Channels let you interact with a Doct Code agent from messaging platforms like Telegram, WeChat, or DingTalk, instead of the terminal. You send messages from your phone or desktop chat app, and the agent responds just like it would in the CLI.

## How It Works

When you run `doct channel start`, Doct Code:
...
2. Add the channel configuration to `~/.doct/settings.json`
3. Run `doct channel start` to start all channels, or `doct channel start <name>` for a single channel
...
| `model`                  | No       | Model to use for this channel (e.g., `doct3.5-plus`). Overrides the default model. Useful for multimodal models that support image input       |
...
```bash
doct channel pairing approve my-channel VEQDDWXJ
```

Once approved, the user's ID is saved to `~/.doct/channels/<name>-allowlist.json` and all future messages go through normally.

### Pairing CLI Commands

```bash
# List pending pairing requests
doct channel pairing list my-channel

# Approve a request by code
doct channel pairing approve my-channel <CODE>
```
...
- Approved users are stored in `~/.doct/channels/<name>-allowlist.json` — treat this file as sensitive
...
The bot runs in the foreground. Press `Ctrl+C` to stop, or use `doct channel stop` from another terminal.

### Multi-Channel Mode

When you run `doct channel start` without a name, all channels defined in `settings.json` start together sharing a single agent process. Each channel maintains its own sessions — a Telegram user and a WeChat user get separate conversations, even though they share the same agent.

Each channel uses its own `cwd` from its config, so different channels can work on different projects simultaneously.

### Service Management

The channel service uses a PID file (`~/.doct/channels/service.pid`) to track the running instance:

- **Duplicate prevention**: Running `doct channel start` while a service is already running will show an error instead of starting a second instance
- **`doct channel stop`**: Gracefully stops the running service from another terminal
- **`doct channel status`**: Shows whether the service is running, its uptime, and session counts per channel
...
- Sessions are persisted to `~/.doct/channels/sessions.json` while the service is running
...
- On clean shutdown (Ctrl+C or `doct channel stop`): session data is cleared — the next start is always fresh
