...
You can extend the channel system with custom platform adapters packaged as [extensions](../../extension/introduction). This lets you connect Doct Code to any messaging platform, webhook, or custom transport.

## How It Works

Channel plugins are loaded at startup from active extensions. When `doct channel start` runs, it:

1. Scans all enabled extensions for `channels` entries in their `doct-extension.json`
...
doct extensions install /path/to/my-channel-extension

# Or link it for development (changes are reflected immediately)
doct extensions link /path/to/my-channel-extension
```

## Configuring a Custom Channel

Add a channel entry to `~/.doct/settings.json` using the custom type provided by the extension:
