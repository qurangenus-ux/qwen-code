...
```
/arena --models doct3.5-plus,glm-5,kimi-k2.5 "Refactor the authentication module to use JWT tokens"
```
...
1. **Worktree setup**: Doct Code creates isolated Git worktrees for each agent at `~/.doct/arena/<session-id>/worktrees/<model-name>/`. Each worktree mirrors your current working directory state exactly — including staged changes, unstaged changes, and untracked files.
...
| :------------------------ | :--------------------------------- | :-------------- |
| `arena.worktreeBaseDir`   | Base directory for arena worktrees | `~/.doct/arena` |
...
```
/arena --models doct3.5-plus,glm-5,kimi-k2.5 "Optimize the database query layer"
```
...
- Ensure you have write access to the worktree base directory (`~/.doct/arena/` by default)
...
Agent Arena is one of several planned multi-agent modes in Doct Code. **Agent Team** and **Agent Swarm** are not yet implemented — the table below describes their intended design for reference.
...
- **Manual parallel sessions**: Run multiple Doct Code sessions yourself in separate terminals with [Git worktrees](https://git-scm.com/docs/git-worktree) for full manual control
