...
This document explains how to run Doct Code inside a sandbox to reduce risk when tools execute shell commands or modify files.

## Prerequisites

Before using sandboxing, you need to install and set up Doct Code:
...
> [!note]
>
> **Naming note:** Some sandbox-related environment variables may have used the `GEMINI_*` prefix historically. All new environment variables use the `DOCT_*` prefix.
...
By default, Doct Code uses a published sandbox image (configured in the CLI package) and will pull it as needed.

The container sandbox mounts your workspace and your `~/.doct` directory into the container so auth and settings persist between runs.
...
doct -s -p "analyze the code structure"

# Or enable sandboxing for your shell session (recommended for CI / scripts)
export DOCT_SANDBOX=true   # true auto-picks a provider (see notes below)
doct -p "run the test suite"
...
>
> - On **macOS**, `DOCT_SANDBOX=true` typically selects `sandbox-exec` (Seatbelt) if available.
> - On **Linux/Windows**, `DOCT_SANDBOX=true` requires `docker` or `podman` to be installed.
> - To force a provider, set `DOCT_SANDBOX=docker|podman|sandbox-exec`.

## Configuration

### Enable sandboxing (in order of precedence)

1. **Environment variable**: `DOCT_SANDBOX=true|false|docker|podman|sandbox-exec`
...
> [!important]
>
> If `DOCT_SANDBOX` is set, it **overrides** the CLI flag and `settings.json`.

### Configure the sandbox image (Docker/Podman)

- **CLI flag**: `--sandbox-image <image>`
- **Environment variable**: `DOCT_SANDBOX_IMAGE=<image>`
- **Settings file**: `tools.sandboxImage` in your `settings.json` (e.g., `{"tools": {"sandboxImage": "ghcr.io/doctlm/doct-code:0.14.1"}}`)

Priority order (highest to lowest):

1. `--sandbox-image`
2. `DOCT_SANDBOX_IMAGE`
3. `tools.sandboxImage`
4. Built-in default image from the CLI package (for example `ghcr.io/doctlm/doct-code:<version>`)

`settings.env.DOCT_SANDBOX_IMAGE` also works as a generic env injection mechanism, but `tools.sandboxImage` is the preferred persistent setting.
...
1. Create a file named `.doct/sandbox-macos-<profile_name>.sb` in your project.
...
- Set `DOCT_SANDBOX_PROXY_COMMAND=<command>`
...
On Linux, Doct Code defaults to enabling UID/GID mapping so the sandbox runs as your user (and reuses the mounted `~/.doct`). Override with:
...
- Container sandbox: add them via `.doct/sandbox.Dockerfile` or `.doct/sandbox.bashrc`.
...
The official Doct Code Docker image is intentionally minimal to keep the image small, secure, and fast to pull. Different users require different language runtimes (Java, Python, Node.js, etc.), and bundling all environments into a single image is not practical. Therefore, Java is **not included by default** in the Docker sandbox.

If your workflow requires Java, you can extend the base image by creating a `.doct/sandbox.Dockerfile` in your project:

```dockerfile
FROM ghcr.io/doctlm/doct-code:latest
...
```bash
DOCT_SANDBOX=docker BUILD_SANDBOX=1 doct -s
```
...
```bash
DEBUG=1 doct -s -p "debug command"
```

**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect the CLI due to automatic exclusion. Use `.doct/.env` files for Doct Code-specific debug settings.

### Inspect sandbox

```bash
# Check environment
doct -s -p "run shell command: env | grep SANDBOX"

# List mounts
doct -s -p "run shell command: mount | grep workspace"
```
