...
1. **DashScope** (Official, Free) - Automatically available for Doct OAuth users (200 requests/minute, 100 requests/day; free tier ending 2026-04-15)
...
- **Doct OAuth users:** DashScope is automatically added to your provider list, even if not explicitly configured
...
Pass API keys when running Doct Code:

```bash
# Tavily
doct --tavily-api-key tvly-xxxxx

# Google
doct --google-api-key your-key --google-search-engine-id your-id

# Specify default provider
doct --web-search-default tavily
```
...
**Note:** This setting requires a restart of Doct Code to take effect. Once disabled, the `web_search` tool will not be available to the model, even if web search providers are configured.
...
- **Cost:** Free
- **Authentication:** Automatically available when using Doct OAuth authentication
- **Configuration:** No API key required, automatically added to provider list for Doct OAuth users
- **Quota:** 200 requests/minute, 100 requests/day (free tier ending 2026-04-15)
- **Best for:** General queries, always available as fallback for Doct OAuth users
- **Auto-registration:** If you're using Doct OAuth, DashScope is automatically added to your provider list even if you don't configure it explicitly
...
- **DashScope availability:** Automatically available for Doct OAuth users, no configuration needed
...
- **For Doct OAuth users:** The tool is automatically registered with DashScope provider, no configuration needed
...
- Check your configuration: Run `doct` and use the settings dialog
- View your current settings in `~/.doct-code/settings.json` (macOS/Linux) or `%USERPROFILE%\.doct-code\settings.json` (Windows)
