...
This document describes the `todo_write` tool for Doct Code.
...
## How to use `todo_write` with Doct Code
...
The tool stores todo lists in your home directory (`~/.doct/todos/`) with session-specific files, so each coding session maintains its own task list.
