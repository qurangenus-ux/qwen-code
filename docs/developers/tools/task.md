...
This document describes the `task` tool for Doct Code.
...
## How to use `task` with Doct Code
...
You can view available subagents by using the `/agents` command in Doct Code.
...
Subagents are configured through Doct Code's agent configuration system. Use the `/agents` command to:
