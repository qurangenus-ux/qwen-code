...
import { getProjectHash } from '@doct-code/doct-code-core/src/utils/paths.js';
import { truncatePanelTitle } from '../webview/utils/panelTitleUtils.js';

export interface DoctMessage {
  id: string;
  timestamp: string;
  type: 'user' | 'doct';
  content: string;
...
export interface DoctSession {
  sessionId: string;
  projectHash: string;
  startTime: string;
  lastUpdated: string;
  messages: DoctMessage[];
  filePath?: string;
  messageCount?: number;
...
export class DoctSessionReader {
  private doctDir: string;

  constructor() {
    this.doctDir = path.join(os.homedir(), '.doct');
  }
  ...
  async getAllSessions(
    ...
    try {
      const sessions: DoctSession[] = [];

      if (!allProjects && workingDir) {
        // Current project only
        const projectHash = getProjectHash(workingDir);
        const chatsDir = path.join(this.doctDir, 'tmp', projectHash, 'chats');
        const projectSessions = await this.readSessionsFromDir(chatsDir);
        sessions.push(...projectSessions);
      } else {
        // All projects
        const tmpDir = path.join(this.doctDir, 'tmp');
        if (!fs.existsSync(tmpDir)) {
          console.log('[DoctSessionReader] Tmp directory not found:', tmpDir);
          return [];
        }
      ...
      console.error('[DoctSessionReader] Failed to get sessions:', error);
      return [];
    }
  }
   ...
   */
  private async readSessionsFromDir(chatsDir: string): Promise<DoctSession[]> {
    const sessions: DoctSession[] = [];
    ...
    for (const file of jsonSessionFiles) {
      ...
      try {
        ...
        const session = JSON.parse(content) as DoctSession;
        session.filePath = filePath;
        sessions.push(session);
      } catch (error) {
        console.error(
          '[DoctSessionReader] Failed to read session file:',
          filePath,
          error,
        );
      }
    ...
    for (const file of jsonlSessionFiles) {
      ...
      try {
        ...
        console.error(
          '[DoctSessionReader] Failed to read JSONL session file:',
          filePath,
          error,
        );
      }
   ...
   */
  getSessionTitle(session: DoctSession): string {
  ...
  private async readJsonlSession(
    ...
    try {
      ...
      console.error(
        '[DoctSessionReader] Failed to parse JSONL session:',
        error,
      );
      return null;
    }
  ...
  async deleteSession(
    ...
    try {
      ...
      console.error('[DoctSessionReader] Failed to delete session:', error);
      return false;
    }
  }
}
