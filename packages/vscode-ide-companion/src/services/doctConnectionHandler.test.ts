...
import { DoctConnectionHandler } from './doctConnectionHandler.js';
import type { AcpConnection } from './acpConnection.js';

describe('DoctConnectionHandler', () => {
  let handler: DoctConnectionHandler;
  ...
  beforeEach(async () => {
    ...
    handler = new DoctConnectionHandler();
    mockConnection = {
    ...
    it('retries connect on spawn failure and succeeds on second attempt', async () => {
      (mockConnection.connect as ReturnType<typeof vi.fn>)
        .mockRejectedValueOnce(
          new Error(
            'Doct ACP process failed to start (exit code: null, signal: SIGTERM)',
          ),
        )
