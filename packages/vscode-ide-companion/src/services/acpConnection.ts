...
export class AcpConnection {
  ...
  async connect(
    ...
    if (!fs.existsSync(cliEntryPath)) {
      throw new Error(
        `Bundled Doct CLI entry not found at ${cliEntryPath}. The extension may not have been packaged correctly.`,
      );
    }
  ...
  private async setupChildProcessHandlers(): Promise<void> {
    ...
    this.child!.stderr?.on('data', (data: Buffer) => {
      ...
      if (
        ...
        console.error(`[ACP doct]:`, message);
      } else {
        console.log(`[ACP doct]:`, message);
      }
    });
    ...
    this.child!.on('exit', (code: number | null, signal: string | null) => {
      console.error(
        `[ACP doct] Process exited with code: ${code}, signal: ${signal}`,
      );
      ...
      rejectOnExit?.(
        new Error(
          `Doct ACP process exited unexpectedly (exit code: ${code}, signal: ${signal})${stderrSuffix}`,
        ),
      );
    ...
    if (!this.child || this.child.killed) {
      ...
      const stderrSuffix = stderrOutput
        ...
        : '';
      throw new Error(
        `Doct ACP process failed to start (exit code: ${code}, signal: ${signal})${stderrSuffix}`,
      );
    }
