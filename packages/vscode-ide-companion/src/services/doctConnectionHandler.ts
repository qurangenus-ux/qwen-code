...
/**
 * Doct Connection Handler
 *
 * Handles Doct Agent connection establishment, authentication, and session creation
 */
...
export interface DoctConnectionResult {
...
/**
 * Doct Connection Handler class
 * Handles connection, authentication, and session initialization
 */
export class DoctConnectionHandler {
  /**
   * Connect to Doct service and establish session
   *
  ...
  async connect(
    ...
    console.log(`[DoctAgentManager] 🚀 CONNECT() CALLED - ID: ${connectId}`);
    ...
    if (proxyUrl) {
      ...
      console.log(
        '[DoctAgentManager] Using proxy from VSCode settings:',
        proxyUrl,
      );
    }
    ...
    for (let attempt = 1; attempt <= maxConnectAttempts; attempt++) {
      try {
        console.log(
          `[DoctAgentManager] Connecting to ACP process (attempt ${attempt}/${maxConnectAttempts})...`,
        );
        await connection.connect(cliEntryPath!, workingDir, extraArgs);
        console.log('[DoctAgentManager] ACP process connected successfully');
        break;
      } catch (connectError) {
        console.error(
          `[DoctAgentManager] Connect attempt ${attempt} failed:`,
        ...
        console.log(`[DoctAgentManager] Retrying connect in ${delay}ms...`);
    ...
    if (!sessionRestored) {
      console.log(
        '[DoctAgentManager] no sessionRestored, Creating new session...',
      );

      try {
        console.log(
          '[DoctAgentManager] Creating new session (letting CLI handle authentication)...',
        );
        ...
        if (
          ...
          console.log(
            '[DoctAgentManager] Extracted availableModels from session/new:',
        ...
        console.log('[DoctAgentManager] New session created successfully');
        sessionCreated = true;
      } catch (sessionError) {
        const needsAuth =
        ...
        if (needsAuth) {
          requiresAuth = true;
          console.log(
            '[DoctAgentManager] Session creation requires authentication; waiting for user-triggered login.',
          );
        } else {
          console.log(
            `\n⚠️ [SESSION FAILED] newSessionWithRetry threw error\n`,
          );
          console.log(`[DoctAgentManager] Error details:`, sessionError);
          throw sessionError;
        }
      }
    } else {
    ...
    console.log(`\n========================================`);
    console.log(`[DoctAgentManager] ✅ CONNECT() COMPLETED SUCCESSFULLY`);
    console.log(`========================================\n`);
    return {
      sessionCreated,
      requiresAuth,
  ...
  private async newSessionWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(
          `[DoctAgentManager] Creating session (attempt ${attempt}/${maxRetries})...`,
        );
        const res = await connection.newSession(workingDir);
        console.log('[DoctAgentManager] Session created successfully');
        return res;
      } catch (error) {
        lastError = error;
        const errorMessage = getErrorMessage(error);
        console.error(
          `[DoctAgentManager] Session creation attempt ${attempt} failed:`,
          errorMessage,
        );

        // If Doct reports that authentication is required, try to
        // authenticate on-the-fly once and retry without waiting.
        const requiresAuth = isAuthenticationRequiredError(error);
        if (requiresAuth) {
          if (!autoAuthenticate) {
            console.log(
              '[DoctAgentManager] Authentication required but auto-authentication is disabled. Propagating error.',
            );
            throw error;
          }
          console.log(
            '[DoctAgentManager] Doct requires authentication. Authenticating and retrying session/new...',
          );
          try {
            ...
            console.log(
              '[DoctAgentManager] newSessionWithRetry Authentication successful',
            );
            // Retry immediately after successful auth
            const res = await connection.newSession(workingDir);
            console.log(
              '[DoctAgentManager] Session created successfully after auth',
            );
            return res;
          } catch (authErr) {
            console.error(
        ...
        console.log(`[DoctAgentManager] Retrying in ${delay}ms...`);
        await new Promise((resolve) => setTimeout(resolve, delay));
      }
    }
