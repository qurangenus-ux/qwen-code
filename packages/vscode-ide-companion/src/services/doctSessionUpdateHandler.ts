...
/**
 * Doct Session Update Handler class
 * Processes various session update events and calls appropriate callbacks
 */
export class DoctSessionUpdateHandler {
  private callbacks: DoctAgentCallbacks;

  constructor(callbacks: DoctAgentCallbacks) {
   ...
   */
  updateCallbacks(callbacks: DoctAgentCallbacks): void {
    this.callbacks = callbacks;
  }
  ...
  handleSessionUpdate(data: SessionNotification): void {
    ...
    switch (sessionUpdate) {
      ...
      default:
        console.log('[DoctAgentManager] Unhandled session update type');
        break;
    }
  }
