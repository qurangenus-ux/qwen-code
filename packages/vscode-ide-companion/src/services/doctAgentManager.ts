...
import { DoctSessionReader, type DoctSession } from './doctSessionReader.js';
import { DoctSessionManager } from './doctSessionManager.js';
import type {
  ChatMessage,
  PlanEntry,
  ToolCallUpdateData,
...
import {
  DoctConnectionHandler,
  type DoctConnectionResult,
} from '../services/doctConnectionHandler.js';
import { DoctSessionUpdateHandler } from './doctSessionUpdateHandler.js';
...
export class DoctAgentManager {
  private connection: AcpConnection;
  private sessionReader: DoctSessionReader;
  private sessionManager: DoctSessionManager;
  private connectionHandler: DoctConnectionHandler;
  private sessionUpdateHandler: DoctSessionUpdateHandler;
  ...
  // Callback storage
  private callbacks: DoctAgentCallbacks = {};
  ...
  constructor() {
    ...
    this.sessionReader = new DoctSessionReader();
    this.sessionManager = new DoctSessionManager();
    this.connectionHandler = new DoctConnectionHandler();
    this.sessionUpdateHandler = new DoctSessionUpdateHandler({});
    ...
    this.connection.onSessionUpdate = (data: SessionNotification) => {
      ...
      try {
        ...
        console.warn('[DoctAgentManager] Rehydration routing failed:', err);
      }
    ...
    this.connection.onEndTurn = (reason?: string) => {
      try {
        ...
        console.warn('[DoctAgentManager] onEndTurn callback error:', err);
      }
    };
    ...
    this.connection.onAuthenticateUpdate = (
      ...
      try {
        ...
        console.warn(
          '[DoctAgentManager] onAuthenticateUpdate callback error:',
          err,
        );
      }
    };
    ...
    this.connection.onInitialized = (init: unknown) => {
      try {
        ...
        console.warn('[DoctAgentManager] onInitialized parse error:', err);
      }
    };
    ...
    this.connection.onDisconnected = (
      ...
      console.log(
        `[DoctAgentManager] Process disconnected (code: ${code}, signal: ${signal})`,
      );
  ...
  async connect(
    ...
    if (res.availableModels && res.availableModels.length > 0) {
      ...
      console.log(
        '[DoctAgentManager] Emitting availableModels from connect():',
  ...
  async reconnect(
    ...
    console.log('[DoctAgentManager] Attempting reconnection...');
    try {
  ...
  async setApprovalModeFromUi(
    ...
    try {
      ...
      console.error('[DoctAgentManager] Failed to set mode:', err);
      throw err;
    }
  }
  ...
  async setModelFromUi(modelId: string): Promise<ModelInfo | null> {
    try {
      ...
      console.error('[DoctAgentManager] Failed to set model:', err);
      throw err;
    }
  }
  ...
  async validateCurrentSession(): Promise<boolean> {
    try {
      ...
      console.warn('[DoctAgentManager] Session validation failed:', error);
  ...
  async getSessionList(): Promise<Array<Record<string, unknown>>> {
    console.log(
      '[DoctAgentManager] Getting session list with version-aware strategy',
    );

    try {
      console.log(
        '[DoctAgentManager] Attempting to get session list via ACP method',
      );
      const response = await this.connection.listSessions();
      console.log('[DoctAgentManager] ACP session list response:', response);

      const res: unknown = response;
      const items = extractSessionListItems(res);

      console.log(
        '[DoctAgentManager] Sessions retrieved via ACP:',
        res,
        items.length,
      );
      if (items.length > 0) {
      ...
      console.warn(
        '[DoctAgentManager] ACP session list failed, falling back to file system method:',
        error,
      );
    }
    ...
    try {
      console.log('[DoctAgentManager] Getting session list from file system');
      const sessions = await this.sessionReader.getAllSessions(undefined, true);
      console.log(
        '[DoctAgentManager] Session list from file system (all projects):',
        sessions.length,
      );

      const result = sessions.map(
        (session: DoctSession): Record<string, unknown> => ({
      ...
      console.log(
        '[DoctAgentManager] Sessions retrieved from file system:',
        result.length,
      );
      return result;
    } catch (error) {
      console.error(
        '[DoctAgentManager] Failed to get session list from file system:',
        error,
      );
      return [];
    }
  ...
  async getSessionListPaged(params?: {
    ...
    try {
      ...
      console.warn('[DoctAgentManager] Paged ACP session list failed:', error);
    ...
    try {
      ...
      console.error('[DoctAgentManager] File system paged list failed:', error);
  ...
  async getSessionMessages(sessionId: string): Promise<ChatMessage[]> {
    try {
      try {
        ...
        console.log(
          '[DoctAgentManager] Session list item for filePath lookup:',
          item,
        );
        if (
          typeof item === 'object' &&
        ...
        console.warn('[DoctAgentManager] JSONL read path lookup failed:', e);
      }
      ...
      console.error(
        '[DoctAgentManager] Failed to get session messages:',
        error,
      );
      return [];
    }
  ...
  private async readJsonlMessages(filePath: string): Promise<ChatMessage[]> {
    ...
    try {
      ...
      console.log(
        '[DoctAgentManager] JSONL records read:',
        records.length,
        filePath,
      );
      ...
      console.log(
        '[DoctAgentManager] JSONL messages reconstructed:',
        msgs.length,
      );
      return msgs;
    } catch (err) {
      console.warn('[DoctAgentManager] Failed to read JSONL messages:', err);
      return [];
    }
  }
  ...
  async loadSessionViaAcp(
    ...
    try {
      ...
      console.log(
        '[DoctAgentManager] Rehydration start for session:',
        sessionId,
      );
      console.log(
        '[DoctAgentManager] Attempting session/load via ACP for session:',
        sessionId,
      );
      ...
      console.log(
        '[DoctAgentManager] Session load succeeded. Response:',
      ...
      console.error(
        '[DoctAgentManager] Session load via ACP failed for session:',
        sessionId,
      );
      console.error('[DoctAgentManager] Error type:', error?.constructor?.name);
      console.error('[DoctAgentManager] Error message:', errorMessage);
      ...
      if (error && typeof error === 'object') {
        ...
        if ('error' in error) {
          ...
          if (acpError.error) {
            console.error(
              '[DoctAgentManager] ACP error code:',
              acpError.error.code,
            );
            console.error(
              '[DoctAgentManager] ACP error message:',
              acpError.error.message,
            );
          }
        } else {
          console.error('[DoctAgentManager] Non-ACPIf error details:', error);
        }
      }
      ...
      console.log('[DoctAgentManager] Rehydration end for session:', sessionId);
  ...
  async loadSession(sessionId: string): Promise<ChatMessage[] | null> {
    console.log(
      '[DoctAgentManager] Loading session with version-aware strategy:',
      sessionId,
    );

    try {
      console.log(
        '[DoctAgentManager] Attempting to load session via ACP method',
      );
      await this.loadSessionViaAcp(sessionId);
      console.log('[DoctAgentManager] Session loaded successfully via ACP');
      ...
      console.warn(
        '[DoctAgentManager] ACP session load failed, falling back to file system method:',
        error,
      );
    }
    ...
    try {
      console.log(
        '[DoctAgentManager] Loading session messages from file system',
      );
      const messages = await this.loadSessionMessagesFromFile(sessionId);
      console.log(
        '[DoctAgentManager] Session messages loaded successfully from file system',
      );
      return messages;
    } catch (error) {
      console.error(
        '[DoctAgentManager] Failed to load session messages from file system:',
        error,
      );
      return null;
    }
  ...
  private async loadSessionMessagesFromFile(
    ...
    try {
      console.log(
        '[DoctAgentManager] Loading session from file system:',
        sessionId,
      );
      ...
      if (!session) {
        console.log(
          '[DoctAgentManager] Session not found in file system:',
          sessionId,
        );
        return null;
      }
      ...
      console.error(
        '[DoctAgentManager] Session load from file system failed:',
        error,
      );
      throw error;
    }
  ...
  async createNewSession(
    ...
    if (!forceNew && this.connection.currentSessionId) {
      console.log(
        '[DoctAgentManager] createNewSession: reusing existing session',
    ...
    if (this.sessionCreateInFlight) {
      console.log(
        '[DoctAgentManager] createNewSession: session creation already in flight',
      );
      if (!forceNew) {
    ...
    console.log('[DoctAgentManager] Creating new session...');

    this.sessionCreateInFlight = (async () => {
      try {
        let newSessionResult: unknown;
        // Try to create a new ACP session. If Doct asks for auth, let it handle authentication.
        try {
          newSessionResult = await this.connection.newSession(workingDir);
          console.log(
            '[DoctAgentManager] newSession returned:',
          ...
          if (requiresAuth) {
            if (!autoAuthenticate) {
              console.warn(
                '[DoctAgentManager] session/new requires authentication but auto-auth is disabled. Deferring until user logs in.',
              );
              throw err;
            }
            console.warn(
              '[DoctAgentManager] session/new requires authentication. Retrying with authenticate...',
            );
            try {
              // Let CLI handle authentication - it's the single source of truth
              await this.connection.authenticate(authMethod);
              console.log(
                '[DoctAgentManager] createNewSession Authentication successful. Retrying session/new...',
              );
              ...
              console.error(
                '[DoctAgentManager] Re-authentication failed:',
                reauthErr,
              );
              throw reauthErr;
            }
        ...
        console.log(
          '[DoctAgentManager] New session created with ID:',
          newSessionId,
        );
        return newSessionId;
      } finally {
  ...
  async cancelCurrentPrompt(): Promise<void> {
    console.log('[DoctAgentManager] Cancelling current prompt');
