...
import {
  ...
  IDE_DEFINITIONS,
  type IdeInfo,
} from '@doct-code/doct-code-core/src/ide/detect-ide.js';
...
const CLI_IDE_COMPANION_IDENTIFIER = 'doctlm.doct-code-vscode-ide-companion';
const INFO_MESSAGE_SHOWN_KEY = 'doctCodeInfoMessageShown';
export const DIFF_SCHEME = 'doct-diff';
  ...
  try {
    ...
    if (latestVersion && semver.gt(latestVersion, currentVersion)) {
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the Doct Code Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('Doct Code Companion');
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer('doctCode.chat', {
  ...
  context.subscriptions.push(
    ...
    (vscode.commands.registerCommand('doct.diff.accept', (uri?: vscode.Uri) => {
    ...
    vscode.commands.registerCommand('doct.diff.cancel', (uri?: vscode.Uri) => {
      const docUri = uri ?? vscode.window.activeTextEditor?.document.uri;
      if (docUri && docUri.scheme === DIFF_SCHEME) {
    ...
    vscode.commands.registerCommand('doct.diff.closeAll', async () => {
      try {
        await diffManager.closeAll();
      } catch (err) {
        console.warn('[Extension] doct.diff.closeAll failed:', err);
      }
    }),
    vscode.commands.registerCommand('doct.diff.suppressBriefly', async () => {
      try {
        diffManager.suppressFor(1200);
      } catch (err) {
        console.warn('[Extension] doct.diff.suppressBriefly failed:', err);
      }
    }),
  );
  ...
  if (!context.globalState.get(INFO_MESSAGE_SHOWN_KEY) && infoMessageEnabled) {
    void vscode.window.showInformationMessage(
      'Doct Code Companion extension successfully installed.',
    );
      ...
      async (
        ...
        if (!workspaceFolders || workspaceFolders.length === 0) {
          vscode.window.showInformationMessage(
            'No folder open. Please open a folder to run Doct Code.',
          );
          return;
        }
        ...
        if (workspaceFolders.length === 1) {
          ...
          selectedFolder = await vscode.window.showWorkspaceFolderPick({
            placeHolder: 'Select a folder to run Doct Code in',
          });
        }
        ...
        if (selectedFolder) {
          ...
          const terminalOptions: vscode.TerminalOptions = {
            name: `Doct Code (${selectedFolder.name})`,
          ...
          if (isWindows) {
            ...
            doctCmd = `ELECTRON_RUN_AS_NODE=1 ${baseCmd}`;
          }

          const terminal = vscode.window.createTerminal(terminalOptions);
          terminal.show();
          terminal.sendText(doctCmd);
        }
      },
    ),
    vscode.commands.registerCommand('doct-code.showNotices', async () => {
