...
vi.mock('@doct-code/doct-code-core/src/ide/detect-ide.js', () => ({
...
const getPortFromMock = (
  ...
  const port = vi
    .mocked(replaceMock)
    .mock.calls.find((call) => call[0] === 'DOCT_CODE_IDE_SERVER_PORT')?.[1];
...
describe('IDEServer', () => {
  ...
  it('should clear env vars and delete lock file on stop', async () => {
    ...
    const replaceMock = mockContext.environmentVariableCollection.replace;
    const port = getPortFromMock(replaceMock);
    const lockFile = path.join('/home/test', '.doct', 'ide', `${port}.lock`);
