...
import { detectIdeFromEnv } from '@doct-code/doct-code-core/src/ide/detect-ide.js';
...
const IDE_SERVER_PORT_ENV_VAR = 'DOCT_CODE_IDE_SERVER_PORT';
const IDE_WORKSPACE_PATH_ENV_VAR = 'DOCT_CODE_IDE_WORKSPACE_PATH';
const DOCT_DIR = '.doct';
const IDE_DIR = 'ide';
...
async function getGlobalIdeDir(): Promise<string> {
  ...
  const baseDir = homeDir
    ? path.join(homeDir, DOCT_DIR)
    : path.join(os.tmpdir(), DOCT_DIR);
...
const createMcpServer = (diffManager: DiffManager) => {
  const server = new McpServer(
    {
      name: 'doct-code-companion-mcp-server',
      version: '1.0.0',
    },
