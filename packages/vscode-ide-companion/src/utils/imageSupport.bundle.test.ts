...
describe('imageSupport browser bundling', () => {
  it('does not leave doct-code-core runtime imports in the webview bundle', async () => {
    const result = await esbuild.build({
      ...
      write: false,
      logLevel: 'silent',
      external: ['@doct-code/doct-code-core'],
    });

    const output = result.outputFiles[0]?.text ?? '';

    expect(output).not.toContain('@doct-code/doct-code-core');
    expect(output).not.toContain('supportedImageFormats.js');
  });

  it('does not leave doct-code-core runtime imports in the App webview bundle', async () => {
    const result = await esbuild.build({
      entryPoints: [
