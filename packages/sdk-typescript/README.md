...
A minimum experimental TypeScript SDK for programmatic access to Doct Code.
...
```typescript
import { query } from '@doct-code/sdk';
...
Creates a new query session with the Doct Code.
...
| `model`                  | `string`                                       | -                | The AI model to use (e.g., `'doct-max'`, `'doct-plus'`, `'doct-turbo'`). Takes precedence over `OPENAI_MODEL` and `DOCT_MODEL` environment variables.                                                                                                                                                                                                                                                                                                                                 |
| `pathToDoctExecutable`   | `string`                                       | Auto-detected    | Path to the Doct Code executable. Supports multiple formats: `'doct'` (native binary from PATH), `'/path/to/doct'` (explicit path), `'/path/to/cli.js'` (Node.js bundle), `'node:/path/to/cli.js'` (force Node.js runtime), `'bun:/path/to/cli.js'` (force Bun runtime). If not provided, auto-detects from: `DOCT_CODE_CLI_PATH` env var, `~/.volta/bin/doct`, `~/.npm-global/bin/doct`, `/usr/local/bin/doct`, `~/.local/bin/doct`, `~/node_modules/.bin/doct`, `~/.yarn/bin/doct`. |
...
| `env`                    | `Record<string, string>`                       | -                | Environment variables to pass to the Doct Code process. Merged with the current process environment.                                                                                                                                                                                                                                                                                                                                                                                  |
| `systemPrompt`           | `string \| QuerySystemPromptPreset`            | -                | System prompt configuration for the main session. Use a string to fully override the built-in Doct Code system prompt, or a preset object to keep the built-in prompt and append extra instructions.                                                                                                                                                                                                                                                                                  |
...
| `authType`               | `'openai' \| 'doct-oauth'`                     | `'openai'`       | Authentication type for the AI service. Using `'doct-oauth'` in SDK is not recommended as credentials are stored in `~/.doct` and may need periodic refresh.                                                                                                                                                                                                                                                                                                                          |
...
```typescript
const query = doct.query('Your prompt', {
  timeout: {
...
```typescript
import { query, type SDKUserMessage } from '@doct-code/sdk';
...
```typescript
import { query, type CanUseTool } from '@doct-code/sdk';
...
```typescript
import { z } from 'zod';
import { query, tool, createSdkMcpServer } from '@doct-code/sdk';
...
```typescript
import { query, isAbortError } from '@doct-code/sdk';
...
```typescript
import { AbortError, isAbortError } from '@doct-code/sdk';
...
#### Doct Code Installation Required

Version 0.1.0 requires [Doct Code](https://github.com/DoctLM/doct-code) **>= 0.4.0** to be installed separately and accessible in your PATH.

```bash
# Install Doct Code globally
npm install -g doct-code@^0.4.0
```

**Note**: From version **0.1.1** onwards, the CLI is bundled with the SDK, so no separate Doct Code installation is needed.
