...
export function query({
  ...
  /**
   * The prompt to send to the Doct Code CLI process.
  ...
  const pathToDoctExecutable = options.pathToDoctExecutable;
...
function validateOptions(options: QueryOptions): SpawnInfo | undefined {
  ...
  try {
    return prepareSpawnInfo(options.pathToDoctExecutable);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new Error(`Invalid pathToDoctExecutable: ${errorMessage}`);
  }
}
