...
import { IdeClient, AuthType } from '@doct-code/doct-code-core';
 ...
 * For bug reports, removes 'doct-' or 'doct-code-' prefixes from sandbox names.
 *
 * @param stripPrefix - Whether to strip 'doct-' prefix (used for bug reports)
 */
export function getSandboxEnv(stripPrefix = false): string {
  ...
  // For bug reports, remove doct- prefix
  if (stripPrefix) {
    return sandbox.replace(/^doct-(?:code-)?/, '');
  }
