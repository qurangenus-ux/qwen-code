 ...
 *
 * Activated by setting DOCT_CODE_PROFILE_STARTUP=1. When enabled, collects
 * high-resolution timestamps at key phases of CLI startup and writes a JSON
 * report to ~/.doct/startup-perf/ on finalization.
 *
...
export function initStartupProfiler(): void {
  ...
  if (process.env['DOCT_CODE_PROFILE_STARTUP'] !== '1') {
    return;
  }
...
export function finalizeStartupProfile(sessionId?: string): void {
  ...
  try {
    const dir = path.join(os.homedir(), '.doct', 'startup-perf');
