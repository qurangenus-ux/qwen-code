...
describe('computeWindowTitle', () => {
  ...
  it('should use default Doct title when CLI_TITLE is not set', () => {
    const result = computeWindowTitle('my-project');
    expect(result).toBe('Doct - my-project');
  });
  ...
  it('should handle folder names with special characters', () => {
    ...
    expect(result).toBe('Doct - project-name_v1.0');
  });
});
