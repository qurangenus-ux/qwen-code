...
import { createDebugLogger, isGitRepository } from '@doct-code/doct-code-core';
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        ...
        childProcess.execSync('brew list -1 | grep -q "^doct-code$"', {
          stdio: 'ignore',
        });
        return {
    ...
    if (realPath.includes('/.pnpm/global')) {
      const updateCommand = 'pnpm add -g @doct-code/doct-code@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add @doct-code/doct-code@latest';
      return {
    ...
    if (realPath.includes('/.bun/bin')) {
      const updateCommand = 'bun add -g @doct-code/doct-code@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g @doct-code/doct-code@latest';
    return {
