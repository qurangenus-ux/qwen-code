...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/DoctLM/doct-code-action/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest doct-code-action release:`,
      _error,
    );
    throw new Error(
      `Unable to determine the latest doct-code-action release on GitHub.`,
    );
  }
};
