...
import type { Config, SandboxConfig } from '@doct-code/doct-code-core';
import { FatalSandboxError } from '@doct-code/doct-code-core';
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'doct-code-sandbox';
const SANDBOX_NETWORK_NAME = 'doct-code-sandbox';
const SANDBOX_PROXY_NAME = 'doct-code-sandbox-proxy';
...
function entrypoint(workdir: string, cliArgs: string[]): string[] {
  ...
  const cliCmd =
    process.env['NODE_ENV'] === 'development'
      ...
      : process.env['DEBUG']
        ? `node --inspect-brk=0.0.0.0:${process.env['DEBUG_PORT'] || '9229'} $(which doct)`
        : 'doct';
...
export async function start_sandbox(
  ...
  if (config.command === 'sandbox-exec') {
    ...
    // start and set up proxy if DOCT_SANDBOX_PROXY_COMMAND is set
    const proxyCommand = process.env['DOCT_SANDBOX_PROXY_COMMAND'];
  ...
  // determine full path for doct-code to distinguish linked vs installed setting
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under doct-code repo
  //
  // note this can only be done with binary linked from doct-code repo
  if (process.env['BUILD_SANDBOX']) {
    if (!gcPath.includes('doct-code/packages/')) {
      throw new FatalSandboxError(
        'Cannot build sandbox using installed Doct Code binary; ' +
          'run `npm link ./packages/cli` under DoctCode-cli repo to switch to linked binary.',
      );
    } else {
      ...
      execSync(
        `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
        {
          ...
          env: {
            ...process.env,
            DOCT_SANDBOX: config.command, // in case sandbox is enabled via flags (see config.ts under cli package)
          },
        },
      );
    }
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ? 'Try running `npm run build:all` or `npm run build:sandbox` under the doct-code repo to build it locally, or check the image name and your network connection.'
        : 'Please check the image name, your network connection, or notify doct-code-dev@service.alibaba.com if the issue persists.';
    throw new FatalSandboxError(
  ...
  // DOCT_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
  const proxyCommand = process.env['DOCT_SANDBOX_PROXY_COMMAND'];
  ...
  const isIntegrationTest =
    process.env['DOCT_CODE_INTEGRATION_TEST'] === 'true';
  let containerName;
  if (isIntegrationTest) {
    containerName = `doct-code-integration-test-${randomBytes(4).toString(
      'hex',
    )}`;
  ...
  // copy DOCT_CODE_TEST_VAR for integration tests
  if (process.env['DOCT_CODE_TEST_VAR']) {
    args.push(
      '--env',
      `DOCT_CODE_TEST_VAR=${process.env['DOCT_CODE_TEST_VAR']}`,
    );
  }
  ...
  // copy OPENAI_API_KEY and related env vars for Doct
  ...
  // tests that need to access host's ~/.doct (e.g., --resume functionality)
  ...
  if (useCurrentUser) {
    ...
    // allowing tests that need to access host's ~/.doct (e.g., --resume) to work.
    ...
    // finally switch to that user to run the doct process. This is
