...
import { DOCT_DIR } from '@doct-code/doct-code-core';
...
describe('Settings Loading and Merging', () => {
  ...
  describe('loadSettings', () => {
    ...
    it('should add version field to V2 settings without version and write to disk', () => {
      ...
      expect(writtenContent.model?.name).toBe('doct-coder');
    ...
    it('should correctly handle partially migrated settings without version field', () => {
      ...
      expect(writtenContent.model).toEqual({ name: 'doct-coder' });
    ...
    describe('when DOCT_CODE_SYSTEM_SETTINGS_PATH is set', () => {
      const MOCK_ENV_SYSTEM_SETTINGS_PATH = '/mock/env/system/settings.json';

      beforeEach(() => {
        process.env['DOCT_CODE_SYSTEM_SETTINGS_PATH'] =
          MOCK_ENV_SYSTEM_SETTINGS_PATH;
      });

      afterEach(() => {
        delete process.env['DOCT_CODE_SYSTEM_SETTINGS_PATH'];
      });
  ...
  describe('loadEnvironment', () => {
    function setup({
      ...
      const geminiEnvPath = path.resolve(path.join(DOCT_DIR, '.env'));
