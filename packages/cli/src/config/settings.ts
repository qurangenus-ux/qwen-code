...
export function getSystemSettingsPath(): string {
  if (process.env['DOCT_CODE_SYSTEM_SETTINGS_PATH']) {
    return process.env['DOCT_CODE_SYSTEM_SETTINGS_PATH'];
  }
  if (platform() === 'darwin') {
    return '/Library/Application Support/DoctCode/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\doct-code\\settings.json';
  } else {
    return '/etc/doct-code/settings.json';
  }
}

export function getSystemDefaultsPath(): string {
  if (process.env['DOCT_CODE_SYSTEM_DEFAULTS_PATH']) {
    return process.env['DOCT_CODE_SYSTEM_DEFAULTS_PATH'];
  }
  return path.join(
...
function findEnvFile(settings: Settings, startDir: string): string | null {
  ...
  const userLevelPaths = new Set([
    ...
    path.normalize(path.join(homeDir, DOCT_DIR, '.env')),
  ]);
  ...
  while (true) {
    // Prefer gemini-specific .env under DOCT_DIR
    const geminiEnvPath = path.join(currentDir, DOCT_DIR, '.env');
    ...
    if (parentDir === currentDir || !parentDir) {
      ...
      const homeGeminiEnvPath = path.join(homeDir, DOCT_DIR, '.env');
...
export function loadEnvironment(settings: Settings): void {
  ...
  if (envFilePath) {
    try {
      ...
      const isProjectEnvFile = !envFilePath.includes(DOCT_DIR);
