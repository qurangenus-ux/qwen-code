...
describe('AuthDialog', () => {
  ...
  describe('GEMINI_API_KEY environment variable', () => {
    ...
    it('should not show the GEMINI_API_KEY message if DOCT_DEFAULT_AUTH_TYPE is set to something else', () => {
      process.env['GEMINI_API_KEY'] = 'foobar';
      process.env['DOCT_DEFAULT_AUTH_TYPE'] = AuthType.USE_OPENAI;
    ...
    it('should show the GEMINI_API_KEY message if DOCT_DEFAULT_AUTH_TYPE is set to use api key', () => {
      process.env['GEMINI_API_KEY'] = 'foobar';
      process.env['DOCT_DEFAULT_AUTH_TYPE'] = AuthType.USE_OPENAI;
  ...
  describe('DOCT_DEFAULT_AUTH_TYPE environment variable', () => {
    it('should select the auth type specified by DOCT_DEFAULT_AUTH_TYPE', () => {
      // DOCT_OAUTH is the only valid AuthType that can be selected via env var
      // API-KEY is not an AuthType enum value, so it cannot be selected this way
      process.env['DOCT_DEFAULT_AUTH_TYPE'] = AuthType.DOCT_OAUTH;
      ...
      // DOCT_OAUTH is the first option, so it should be selected
      expect(lastFrame()).toContain('Doct OAuth');
    });

    it('should fall back to default if DOCT_DEFAULT_AUTH_TYPE is not set', () => {
      const settings: LoadedSettings = new LoadedSettings(
        {
          settings: {
            security: { auth: { selectedType: undefined } },
      ...
      // Default is Doct OAuth (first option)
      expect(lastFrame()).toContain('Doct OAuth');
    });

    it('should show an error and fall back to default if DOCT_DEFAULT_AUTH_TYPE is invalid', () => {
      process.env['DOCT_DEFAULT_AUTH_TYPE'] = 'invalid-auth-type';
      ...
      // Since the auth dialog doesn't show DOCT_DEFAULT_AUTH_TYPE errors anymore,
      // it will just show the default Doct OAuth option
      expect(lastFrame()).toContain('Doct OAuth');
    });
  });
