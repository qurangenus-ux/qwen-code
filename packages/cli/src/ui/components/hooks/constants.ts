...
import { HooksConfigSource, HookEventName } from '@doct-code/doct-code-core';
...
export function getHookExitCodes(eventName: string): HookExitCode[] {
  const exitCodesMap: Record<string, HookExitCode[]> = {
    ...
    [HookEventName.UserPromptSubmit]: [
      { code: 0, description: t('stdout shown to Doct') },
      {
        code: 2,
        description: t(
...
export function getHookShortDescription(eventName: string): string {
  const descriptions: Record<string, string> = {
    ...
    [HookEventName.Stop]: t('Right before Doct Code concludes its response'),
