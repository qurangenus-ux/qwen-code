...
import { AuthType, DEFAULT_DOCT_MODEL } from '@doct-code/doct-code-core';
import type { LoadedSettings } from '../../config/settings.js';
import { SettingScope } from '../../config/settings.js';
import { getFilteredDoctModels } from '../models/availableModels.js';
...
const createMockGetAvailableModelsForAuthType = () =>
  vi.fn((t: AuthType) => {
    if (t === AuthType.DOCT_OAUTH) {
      return getFilteredDoctModels().map((m) => ({
        id: m.id,
        label: m.label,
...
const renderComponent = (
  ...
  const mockConfig = {
    ...
    getModel: vi.fn(() => DEFAULT_DOCT_MODEL),
    setModel: vi.fn().mockResolvedValue(undefined),
    switchModel: vi.fn().mockResolvedValue(undefined),
    getAuthType: vi.fn(() => 'doct-oauth'),
    getAllConfiguredModels: vi.fn(() =>
      getFilteredDoctModels().map((m) => ({
        id: m.id,
        label: m.label,
...
describe('<ModelDialog />', () => {
  ...
  it('passes all model options to DescriptiveRadioButtonSelect', () => {
    ...
    expect(props.items).toHaveLength(getFilteredDoctModels().length);
    // coder-model is the only model and it has vision capability
    expect(props.items[0].value).toBe(
      `${AuthType.DOCT_OAUTH}::${DEFAULT_DOCT_MODEL}`,
    );
    expect(props.showNumbers).toBe(true);
  });

  it('initializes with the model from ConfigContext', () => {
    const mockGetModel = vi.fn(() => DEFAULT_DOCT_MODEL);
    renderComponent(
      {},
      {
    ...
    const doctModels = getFilteredDoctModels();
    const expectedIndex = doctModels.findIndex(
      (m) => m.id === DEFAULT_DOCT_MODEL,
    );
  ...
  it('initializes with default coder model if getModel returns undefined', () => {
    ...
    // When getModel returns undefined, preferredModel falls back to DEFAULT_DOCT_MODEL
  ...
  it('calls config.switchModel and onClose when DescriptiveRadioButtonSelect.onSelect is triggered', async () => {
    ...
    await childOnSelect(`${AuthType.DOCT_OAUTH}::${DEFAULT_DOCT_MODEL}`);
  ...
  it('updates initialIndex when config context changes', () => {
    const mockGetModel = vi.fn(() => DEFAULT_DOCT_MODEL);
    const mockGetAuthType = vi.fn(() => 'doct-oauth');
    const mockSettings = {
      isTrusted: true,
      user: { settings: {} },
      workspace: { settings: {} },
    ...
    // DEFAULT_DOCT_MODEL (coder-model) is at index 0
    expect(mockedSelect.mock.calls[0][0].initialIndex).toBe(0);

    mockGetModel.mockReturnValue(DEFAULT_DOCT_MODEL);
    const newMockConfig = {
      getModel: mockGetModel,
      getAuthType: mockGetAuthType,
    ...
    // Calculate expected index for DEFAULT_DOCT_MODEL dynamically
    const doctModels = getFilteredDoctModels();
    const expectedCoderIndex = doctModels.findIndex(
      (m) => m.id === DEFAULT_DOCT_MODEL,
    );
