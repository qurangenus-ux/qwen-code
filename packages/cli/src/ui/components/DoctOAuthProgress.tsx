...
interface DoctOAuthProgressProps {
  onTimeout: () => void;
  onCancel: () => void;
...
export function DoctOAuthProgress({
  onTimeout,
  onCancel,
  deviceAuth,
  authStatus,
  authMessage,
}: DoctOAuthProgressProps): React.JSX.Element {
    ...
    return (
      ...
      >
        <Text bold color={theme.status.error}>
          {t('Doct OAuth Authentication Timeout')}
        </Text>
  ...
  if (authStatus === 'error') {
    return (
      ...
      >
        <Text bold color={theme.status.error}>
          {t('Doct OAuth Authentication Error')}
        </Text>
  ...
  if (!deviceAuth) {
    return (
      <Box
        ...
        width="100%"
      >
        <Text bold>{t('Doct OAuth Authentication')}</Text>

        <Box marginTop={1} flexDirection="column">
          <Text>{t('Waiting for Doct OAuth authentication...')}</Text>
          <Text>
