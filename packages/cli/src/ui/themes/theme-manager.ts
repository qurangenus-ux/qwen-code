...
import { DoctLight } from './doct-light.js';
import { DoctDark } from './doct-dark.js';
...
export const DEFAULT_THEME: Theme = DoctDark;
...
class ThemeManager {
  ...
  getAvailableThemes(): ThemeDisplay[] {
    ...
    // Separate Doct themes
    const doctThemes = builtInThemes.filter(
      (theme) => theme.name === DoctLight.name || theme.name === DoctDark.name,
    );
    ...
    // Combine Doct themes first, then sorted others
    return [...doctThemes, ...sortedOtherThemes];
  }
