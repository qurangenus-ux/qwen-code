...
export const GITHUB_WORKFLOW_PATHS = [
  'doct-dispatch/doct-dispatch.yml',
  'doct-assistant/doct-invoke.yml',
  'issue-triage/doct-triage.yml',
  'issue-triage/doct-scheduled-triage.yml',
  'pr-review/doct-review.yml',
];
...
// Add Doct Code specific entries to .gitignore file
export async function updateGitignore(gitRepoRoot: string): Promise<void> {
  const gitignoreEntries = ['.doct/', 'gha-creds-*.json'];
...
export const setupGithubCommand: SlashCommand = {
  ...
  action: async (
    ...
    const readmeUrl = `https://github.com/DoctLM/doct-code-action/blob/${releaseTag}/README.md#quick-start`;
    ...
    for (const workflow of GITHUB_WORKFLOW_PATHS) {
      downloads.push(
        (async () => {
          const endpoint = `https://raw.githubusercontent.com/DoctLM/doct-code-action/refs/tags/${releaseTag}/examples/workflows/${workflow}`;
    ...
    return {
      ...
      toolArgs: {
        description:
          'Setting up GitHub Actions to triage issues and review PRs with Doct.',
        command,
        is_background: false,
      },
    };
