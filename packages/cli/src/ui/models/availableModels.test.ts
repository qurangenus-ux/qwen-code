...
describe('availableModels', () => {
  ...
  describe('getAvailableModelsForAuthType', () => {
    ...
    it('should return hard-coded doct models for doct-oauth', () => {
      const models = getAvailableModelsForAuthType(AuthType.DOCT_OAUTH);
      expect(models.length).toBe(1);
      expect(models[0].id).toBe('coder-model');
      expect(models[0].isVision).toBe(true);
    });

    it('should use config models for doct-oauth when config is provided', () => {
      const mockConfig = {
