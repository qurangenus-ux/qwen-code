...
describe('handleAtCommand', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getFileFilteringRespectDoctIgnore: () => true,
  ...
  describe('doct-ignore filtering', () => {
    it('should skip doct-ignored files in @ commands', async () => {
      await createTestFile(
        path.join(testRootDir, '.doctignore'),
        'build/output.js',
      );
      const doctIgnoredFile = await createTestFile(
        path.join(testRootDir, 'build', 'output.js'),
        'console.log("Hello");',
      );
      const query = `@${doctIgnoredFile}`;
      ...
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Path ${doctIgnoredFile} is doct-ignored and will be skipped.`,
      );
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Ignored 1 files:\nDoct-ignored: ${doctIgnoredFile}`,
      );
    });
  });
  it('should process non-ignored files when .doctignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, '.doctignore'),
      'build/output.js',
    );
  ...
  it('should handle mixed doct-ignored and valid files', async () => {
    await createTestFile(
      path.join(testRootDir, '.doctignore'),
      'dist/bundle.js',
    );
    ...
    const query = `@${validFile} @${doctIgnoredFile}`;
    ...
    expect(result).toMatchObject({
      processedQuery: [
        { text: `@${validFile} @${doctIgnoredFile}` },
        { text: '\n--- Content from referenced files ---' },
        { text: `\nContent from ${validFile}:\n` },
