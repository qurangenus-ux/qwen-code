...
import { normalizeProxyUrl } from '@doct-code/doct-code-core';
import { loadSettings } from '../../config/settings.js';
import { writeStderrLine, writeStdoutLine } from '../../utils/stdioHelpers.js';
import { AcpBridge, SessionRouter } from '@doct-code/channel-base';
import type {
  ChannelBase,
  ChannelPlugin,
  ToolCallEvent,
...
function sessionsPath(): string {
  return path.join(os.homedir(), '.doct', 'channels', 'sessions.json');
}
 ...
 * Extensions declare channels in their doct-extension.json manifest.
 */
...
function checkDuplicateInstance(): void {
  ...
  if (existing) {
    ...
    writeStderrLine('Use "doct channel stop" to stop it first.');
    process.exit(1);
  }
}
