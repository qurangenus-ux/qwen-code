...
import { AuthType, OutputFormat } from '@doct-code/doct-code-core';
...
function createMockConfig(overrides?: Partial<Config>): Config {
  const baseModelsConfig = {
    ...
    getCurrentAuthType: vi.fn().mockReturnValue(AuthType.DOCT_OAUTH),
...
describe('validateNonInterActiveAuth', () => {
  ...
  let originalEnvDoctOauth: string | undefined;
  ...
  beforeEach(() => {
    ...
    originalEnvDoctOauth = process.env['DOCT_OAUTH'];
  ...
  afterEach(() => {
    ...
    if (originalEnvDoctOauth !== undefined) {
      process.env['DOCT_OAUTH'] = originalEnvDoctOauth;
    } else {
      delete process.env['DOCT_OAUTH'];
    }
  ...
  it('uses configured DOCT_OAUTH if provided', async () => {
    const nonInteractiveConfig = createMockConfig({
      refreshAuth: refreshAuthMock,
      getModelsConfig: vi.fn().mockReturnValue({
        getModel: vi.fn().mockReturnValue('default-model'),
    ...
    expect(refreshAuthMock).toHaveBeenCalledWith(AuthType.DOCT_OAUTH);
  });
  ...
  it('exits if currentAuthType does not match enforcedAuthType', async () => {
    mockSettings.merged.security!.auth!.enforcedType = AuthType.DOCT_OAUTH;
    process.env['OPENAI_API_KEY'] = 'fake-key';
    const nonInteractiveConfig = createMockConfig({
      refreshAuth: refreshAuthMock,
      getModelsConfig: vi.fn().mockReturnValue({
    ...
    expect(mockWriteStderrLine).toHaveBeenCalledWith(
      'The configured auth type is doct-oauth, but the current auth type is openai. Please re-authenticate with the correct type.',
    );
    expect(processExitSpy).toHaveBeenCalledWith(1);
  });
    ...
    it('emits error result and exits when enforced auth mismatches current auth', async () => {
      ...
      expect(emitResultMock).toHaveBeenCalledWith({
        ...
        errorMessage: expect.stringContaining(
          'The configured auth type is doct-oauth, but the current auth type is openai.',
        ),
        durationMs: 0,
        apiDurationMs: 0,
        numTurns: 0,
