...
The Doct Code Java SDK is a minimum experimental SDK for programmatic access to Doct Code functionality. It provides a Java interface to interact with the Doct Code CLI, allowing developers to integrate Doct Code capabilities into their Java applications.

**Context Information:**

- Current Date: Monday 5 January 2026
- Operating System: darwin
- Working Directory: /Users/weigeng/repos/doct-code/packages/sdk-java

## Project Details

- **Group ID**: com.alibaba
- **Artifact ID**: doctcode-sdk (as per pom.xml)
...
- **API Layer**: Provides the main entry points through `DoctCodeCli` class with simple static methods for basic usage
- **Session Layer**: Manages communication sessions with the Doct Code CLI through the `Session` class
...
- `DoctCodeCli`: Main entry point with static methods for simple queries
...
The project includes basic unit tests using JUnit 5. The main test class `DoctCodeCliTest` demonstrates how to use the SDK to make simple queries to the Doct Code CLI.
...
The `TransportOptions` class allows configuration of how the SDK communicates with the Doct Code CLI:

- `pathToDoctExecutable`: Path to the Doct Code CLI executable
...
- **Session creation**: Use `DoctCodeCli.newSession()` to create a new session with custom options
...
- **Thread Naming**: "doct_code_cli-pool-{number}"
...
The SDK includes several example files in `src/test/java/com/alibaba/doct/code/cli/example/` that demonstrate different aspects of the API:
...
### Q: Do I need to install the Doct CLI separately?
