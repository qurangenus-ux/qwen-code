...
1. Extract components from `vscode-ide-companion/src/webview/` into an independent `@doct-code/webui` package
...
```typescript
// @doct-code/webui/src/context/PlatformContext.ts
...
```javascript
// @doct-code/webui/tailwind.preset.js
module.exports = {
  theme: {
    extend: {
      colors: {
...
```
Developer ──► @doct-code/webui ──► vscode-ide-companion
  │              │                      │
...
// After: Update import
import { FileIcon } from '@doct-code/webui';
// or import { FileIcon } from '@doct-code/webui/icons';
```
...
1. **Tailwind Class Name Tree Shaking**
   ...
   - Solution: Consumer's `content` config needs to include `node_modules/@doct-code/webui`
...
│     └── Edit files in @doct-code/webui/src/                     │
│                                                                  │
...
│     └── import { Component } from '@doct-code/webui'            │
...
1. **Single Source of Truth**: All UI components live in `@doct-code/webui`
