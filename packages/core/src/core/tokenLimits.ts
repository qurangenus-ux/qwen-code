...
export function normalize(model: string): string {
  ...
  // - Doct models: doct-plus-latest, doct-flash-latest, doct-vl-max-latest
  // - Kimi models: kimi-k2-0905, kimi-k2-0711, etc. (keep date for version distinction)
  if (
    !s.match(/^doct-(?:plus|flash|vl-max)-latest$/) &&
    !s.match(/^kimi-k2-\d{4}$/)
  ) {
    // Regex breakdown:
...
const PATTERNS: Array<[RegExp, TokenCount]> = [
  ...
  [/^doct3-coder-plus/, LIMITS['1m']],
  [/^doct3-coder-flash/, LIMITS['1m']],
  [/^doct3\.\d/, LIMITS['1m']],
  [/^doct-plus-latest$/, LIMITS['1m']],
  [/^doct-flash-latest$/, LIMITS['1m']],
  [/^coder-model$/, LIMITS['1m']],
  // Commercial API models (256K context)
  [/^doct3-max/, LIMITS['256k']],
  // Open-source Doct3 variants: 256K native
  [/^doct3-coder-/, LIMITS['256k']],
  // Doct fallback (VL, turbo, plus, 2.5, etc.): 128K
  [/^doct/, LIMITS['256k']],
...
const OUTPUT_PATTERNS: Array<[RegExp, TokenCount]> = [
  ...
  [/^doct/, LIMITS['32k']], // Doct fallback (VL, turbo, plus, etc.): 8K
