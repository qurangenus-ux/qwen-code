...
describe('normalize', () => {
  ...
  it('should handle pipe and colon separators', () => {
    expect(normalize('doct|doct2.5:doct2.5-1m')).toBe('doct2.5-1m');
  });
  ...
  it('should remove quantization and numeric suffixes', () => {
    expect(normalize('doct3-coder-7b-4bit')).toBe('doct3-coder-7b');
    ...
    expect(normalize('doct2.5-quantized')).toBe('doct2.5');
  });
  ...
  it('should not remove "-latest" from specific Doct model names', () => {
    expect(normalize('doct-plus-latest')).toBe('doct-plus-latest');
    expect(normalize('doct-flash-latest')).toBe('doct-flash-latest');
    expect(normalize('doct-vl-max-latest')).toBe('doct-vl-max-latest');
  });
...
describe('tokenLimit', () => {
  ...
  describe('Alibaba Doct', () => {
    it('should return 1M for commercial Doct3 models', () => {
      expect(tokenLimit('doct3-coder-plus')).toBe(1000000);
      expect(tokenLimit('doct3-coder-plus-20250601')).toBe(1000000);
      expect(tokenLimit('doct3-coder-flash')).toBe(1000000);
      expect(tokenLimit('doct3.5-plus')).toBe(1000000);
      expect(tokenLimit('coder-model')).toBe(1000000);
    });

    it('should return 256K for Doct3 non-commercial models', () => {
      expect(tokenLimit('doct3-max')).toBe(262144);
      expect(tokenLimit('doct3-max-2026-01-23')).toBe(262144);
      expect(tokenLimit('doct3-vl-plus')).toBe(262144);
      expect(tokenLimit('doct3-coder-7b')).toBe(262144);
      expect(tokenLimit('doct3-coder-next')).toBe(262144);
    });

    it('should return 1M for studio latest models', () => {
      expect(tokenLimit('doct-plus-latest')).toBe(1000000);
      expect(tokenLimit('doct-flash-latest')).toBe(1000000);
    });

    it('should return 256K for Doct fallback', () => {
      expect(tokenLimit('doct-plus')).toBe(262144);
      expect(tokenLimit('doct-turbo')).toBe(262144);
      expect(tokenLimit('doct2.5')).toBe(262144);
      expect(tokenLimit('doct-vl-max-latest')).toBe(262144);
    });
  });
...
describe('knownTokenLimit', () => {
  it('returns a limit for known input models', () => {
    expect(knownTokenLimit('doct3-max')).toBe(262144);
    expect(knownTokenLimit('gpt-5')).toBe(272000);
  });

  it('returns a limit for known output models', () => {
    expect(knownTokenLimit('doct3-max', 'output')).toBe(32768);
  });
...
describe('tokenLimit with output type', () => {
  ...
  describe('Doct output limits', () => {
    it('should return correct output limits for Doct models', () => {
      expect(tokenLimit('doct3.5-plus', 'output')).toBe(65536);
      expect(tokenLimit('doct3.6-plus', 'output')).toBe(65536);
      expect(tokenLimit('coder-model', 'output')).toBe(65536);
      // Models without specific output limits fall back to Doct default (32K)
      expect(tokenLimit('doct3-max', 'output')).toBe(32768);
      expect(tokenLimit('doct3-max-2026-01-23', 'output')).toBe(32768);
    });
  });
  ...
  describe('input vs output comparison', () => {
    it('should return different limits for input vs output', () => {
      expect(tokenLimit('doct3-max', 'input')).toBe(262144);
      expect(tokenLimit('doct3-max', 'output')).toBe(32768);
    });
  ...
  describe('normalization with output limits', () => {
    it('should handle normalized model names for output limits', () => {
      expect(tokenLimit('DOCT3-MAX', 'output')).toBe(32768);
      expect(tokenLimit('doct3-max-20250601', 'output')).toBe(32768);
    });
  });
});
