...
export class Logger {
  private doctDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.doctDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.doctDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.doctDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.doctDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.doctDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.doctDir) {
      this.debugLogger.error(
