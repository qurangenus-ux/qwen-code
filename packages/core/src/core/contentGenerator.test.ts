...
describe('createContentGeneratorConfig', () => {
  ...
  it('should preserve provided fields and set authType for DOCT_OAUTH', () => {
    const cfg = createContentGeneratorConfig(mockConfig, AuthType.DOCT_OAUTH, {
      model: 'coder-model',
      apiKey: 'DOCT_OAUTH_DYNAMIC_TOKEN',
    });
    expect(cfg.authType).toBe(AuthType.DOCT_OAUTH);
    expect(cfg.model).toBe('coder-model');
    expect(cfg.apiKey).toBe('DOCT_OAUTH_DYNAMIC_TOKEN');
  });

  it('should not warn or fallback for DOCT_OAUTH (resolution handled by ModelConfigResolver)', () => {
    const warnSpy = vi
      .spyOn(console, 'warn')
