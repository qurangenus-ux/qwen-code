...
describe('DashScopeOpenAICompatibleProvider', () => {
  ...
  describe('isDashScopeProvider', () => {
    it('should return true for DOCT_OAUTH auth type', () => {
      const config = {
        authType: AuthType.DOCT_OAUTH,
  ...
  describe('buildHeaders', () => {
    it('should build DashScope-specific headers', () => {
      ...
      expect(headers).toEqual({
        ...
        'X-DashScope-UserAgent': `DoctCode/1.0.0 (${process.platform}; ${process.arch})`,
        'X-DashScope-AuthType': AuthType.DOCT_OAUTH,
      });
    });
    ...
    it('should merge custom headers with DashScope defaults', () => {
      ...
      expect(headers['User-Agent']).toContain('DoctCode/1.0.0');
      expect(headers['X-DashScope-UserAgent']).toContain('DoctCode/1.0.0');
      expect(headers['X-DashScope-AuthType']).toBe(AuthType.DOCT_OAUTH);
  ...
  describe('buildRequest', () => {
    ...
    it('should preserve all original request parameters', () => {
      ...
      expect(result.model).toBe('doct-max');
  ...
  describe('output token limits', () => {
    ...
    it('should set conservative max_tokens default when not present in request', () => {
      ...
      // doct3-max has 32K output limit, so min(32K, 8K) = 8K
