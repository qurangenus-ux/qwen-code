...
/**
 * Converter for Gemini extensions to Doct Code format.
 */
...
/**
 * Converts a Gemini extension config to Doct Code format.
 * @param extensionDir Path to the Gemini extension directory
 * @returns Doct ExtensionConfig
 */
export function convertGeminiToDoctConfig(
  extensionDir: string,
): ExtensionConfig {
...
/**
 * Converts a complete Gemini extension package to Doct Code format.
...
export async function convertGeminiExtensionPackage(
  ...
  const geminiConfig = convertGeminiToDoctConfig(extensionDir);
  ...
  try {
    ...
    // Step 3: Create doct-extension.json with converted config
...
export function isGeminiExtensionConfig(extensionDir: string) {
  ...
  // If it has Gemini-specific fields but not Doct-specific fields, likely Gemini
  return true;
}
