...
export const EXTENSIONS_DIRECTORY_NAME = path.join(DOCT_DIR, 'extensions');
export const EXTENSIONS_CONFIG_FILENAME = 'doct-extension.json';
export const INSTALL_METADATA_FILENAME = '.doct-extension-install.json';
  ...
  try {
    ...
    for (const file of mdFiles) {
      ...
      try {
        ...
        // Replace references to ".claude" directory with ".doct" in markdown files
  ...
  try {
    ...
    for (const file of scriptFiles) {
      ...
      try {
        ...
        // Replace references to ".claude" directory with ".doct" in shell scripts
        // Only match path references (e.g., ~/.claude/, $HOME/.claude, ./.claude/)
        // Avoid matching URLs, comments, or string literals containing .claude
        ...
        if (finalScriptContent !== content) {
          ...
          debugLogger.debug(
            `Updated transcript format and replaced .claude with .doct in shell script: ${filePath}`,
          );
        }
      } catch (error) {
        debugLogger.warn(
