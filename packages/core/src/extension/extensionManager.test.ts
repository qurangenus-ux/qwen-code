...
import { DOCT_DIR } from '../config/storage.js';
import {
  ExtensionManager,
  SettingScope,
...
const EXTENSIONS_DIRECTORY_NAME = path.join(DOCT_DIR, 'extensions');
...
function createExtension({
  ...
  if (addContextFile) {
    fs.writeFileSync(path.join(extDir, 'DOCT.md'), 'context');
  }
...
describe('extension tests', () => {
  ...
  beforeEach(() => {
    tempHomeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'doct-code-test-home-'),
    );
    tempWorkspaceDir = fs.mkdtempSync(
      path.join(tempHomeDir, 'doct-code-test-workspace-'),
    );
  ...
  describe('loadExtension', () => {
    ...
    it('should load context file path when DOCT.md is present', async () => {
      createExtension({
        extensionsDir: userExtensionsDir,
        name: 'ext1',
        version: '1.0.0',
      ...
      expect(ext1?.contextFiles).toEqual([
        path.join(userExtensionsDir, 'ext1', 'DOCT.md'),
      ]);
    ...
    it('should use default DOCT.md when contextFileName is empty array', async () => {
      ...
      fs.writeFileSync(path.join(extDir, 'DOCT.md'), 'context content');
      ...
      expect(ext?.contextFiles).toEqual([
        path.join(userExtensionsDir, 'ext-empty-context', 'DOCT.md'),
      ]);
    });
  ...
  describe('hooks loading and processing', () => {
    it('should load hooks from doct-extension.json', async () => {
      const extensionDir = path.join(userExtensionsDir, 'hooks-extension');
      fs.mkdirSync(extensionDir, { recursive: true });

      // Create doct-extension.json with hooks
    ...
    it('should load hooks from hooks/hooks.json when not in main config', async () => {
      ...
      // Create doct-extension.json without hooks
    ...
    it('should substitute ${CLAUDE_PLUGIN_ROOT} variable in hooks', async () => {
      ...
      // Create doct-extension.json with hooks using ${CLAUDE_PLUGIN_ROOT}
      const configWithHooks = {
    ...
    it('should load hooks from config.hooks string path', async () => {
      ...
      // Create doct-extension.json with hooks as string path
