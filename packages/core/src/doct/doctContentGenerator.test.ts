...
import type { IDoctOAuth2Client } from './doctOAuth2.js';
import { type DoctCredentials, type ErrorData } from './doctOAuth2.js';
import type {
...
import { DoctContentGenerator } from './doctContentGenerator.js';
...
vi.mock('./sharedTokenManager.js', () => ({
  SharedTokenManager: class {
    ...
    private mockCredentials: DoctCredentials | null = null;
    ...
    async getValidCredentials(
      doctClient: IDoctOAuth2Client,
    ): Promise<DoctCredentials> {
      ...
      try {
        const { token } = await doctClient.getAccessToken();
        if (token) {
          const credentials = doctClient.getCredentials();
          return credentials;
        }
      } catch (error) {
        ...
        if (isAuthError) {
          ...
          try {
            const refreshResult = await doctClient.refreshAccessToken();
            if (refreshResult && !('error' in refreshResult)) {
              // Refresh succeeded, update client credentials and return them
              const updatedCredentials = doctClient.getCredentials();
              return updatedCredentials;
            } else {
              // Refresh failed, throw appropriate error
              throw new Error(
                'Failed to obtain valid Doct access token. Please re-authenticate.',
              );
            }
          } catch {
            throw new Error(
    ...
    getCurrentCredentials(): DoctCredentials | null {
      return this.mockCredentials;
    }
    ...
    setMockCredentials(credentials: DoctCredentials | null): void {
...
describe('DoctContentGenerator', () => {
  let mockDoctClient: IDoctOAuth2Client;
  let doctContentGenerator: DoctContentGenerator;
  let mockConfig: Config;

  const mockCredentials: DoctCredentials = {
  ...
  beforeEach(() => {
    ...
    // Create DoctContentGenerator instance
    ...
    doctContentGenerator = new DoctContentGenerator(
      mockDoctClient,
  ...
  describe('Core Content Generation Methods', () => {
    it('should generate content with valid token', async () => {
      vi.mocked(mockDoctClient.getAccessToken).mockResolvedValue({
        token: 'valid-token',
      });
      vi.mocked(mockDoctClient.getCredentials).mockReturnValue(mockCredentials);
      ...
      const result = await doctContentGenerator.generateContent(
        request,
        'test-prompt-id',
      );

      expect(result.text).toBe('Generated content');
      expect(mockDoctClient.getAccessToken).toHaveBeenCalled();
    });
    ...
    it('should generate content stream with valid token', async () => {
      ...
      const stream = await doctContentGenerator.generateContentStream(
        request,
        'test-prompt-id',
      );
      const chunks: string[] = [];
    ...
    it('should count tokens without requiring authentication', async () => {
      ...
      const result = await doctContentGenerator.countTokens(request);

      expect(result.totalTokens).toBe(15);
      // countTokens is a local operation and should not require OAuth credentials
      expect(mockDoctClient.getAccessToken).not.toHaveBeenCalled();
    });
    ...
    it('should embed content with valid token', async () => {
      ...
      const result = await doctContentGenerator.embedContent(request);
  ...
  describe('Token Management and Refresh Logic', () => {
    it('should refresh token on auth error and retry', async () => {
      ...
      vi.mocked(mockDoctClient.getAccessToken)
        .mockRejectedValueOnce(authError)
        .mockResolvedValueOnce({ token: 'refreshed-token' });

      // Refresh succeeds
      vi.mocked(mockDoctClient.refreshAccessToken).mockResolvedValue({
      ...
      vi.mocked(mockDoctClient.getCredentials).mockReturnValue({
        access_token: 'refreshed-token',
        token_type: 'Bearer',
      ...
      expect(result.text).toBe('Generated content');
      expect(mockDoctClient.refreshAccessToken).toHaveBeenCalled();
    });
    ...
    it('should handle token refresh failure', async () => {
      ...
      await expect(
        doctContentGenerator.generateContent(request, 'test-prompt-id'),
      ).rejects.toThrow(
        'Failed to obtain valid Doct access token. Please re-authenticate.',
      );
    ...
    it('should update endpoint when token is refreshed', async () => {
      ...
      await doctContentGenerator.generateContent(request, 'test-prompt-id');

      expect(mockDoctClient.getCredentials).toHaveBeenCalled();
    });
  });
  ...
  describe('Endpoint URL Normalization', () => {
    it('should use default endpoint when no custom endpoint provided', async () => {
      ...
      const parentPrototype = Object.getPrototypeOf(
        Object.getPrototypeOf(doctContentGenerator),
      );
  ...
  describe('Client State Management', () => {
    it('should set dynamic credentials during operations', async () => {
      const client = (
        doctContentGenerator as unknown as {
  ...
  describe('Error Handling and Retry Logic', () => {
    it('should retry once on authentication errors', async () => {
      ...
      vi.mocked(mockDoctClient.getAccessToken).mockImplementation(async () => {
    ...
    it('should not retry non-authentication errors', async () => {
      ...
      expect(mockDoctClient.refreshAccessToken).not.toHaveBeenCalled();
    ...
    it('should handle error response from token refresh', async () => {
      vi.mocked(mockDoctClient.getAccessToken).mockRejectedValue(
      ...
      await expect(
        doctContentGenerator.generateContent(request, 'test-prompt-id'),
      ).rejects.toThrow('Failed to obtain valid Doct access token');
    });
  });

  describe('Token State Management', () => {
    it('should cache and return current token', () => {
      expect(doctContentGenerator.getCurrentToken()).toBeNull();

      // Simulate setting a token internally
      (
        doctContentGenerator as unknown as { currentToken: string }
      ).currentToken = 'cached-token';

      expect(doctContentGenerator.getCurrentToken()).toBe('cached-token');
    });

    it('should clear token on clearToken()', () => {
      // Simulate having cached token value
      const doctInstance = doctContentGenerator as unknown as {
        currentToken: string;
      };
      doctInstance.currentToken = 'cached-token';

      doctContentGenerator.clearToken();
    ...
    it('should handle concurrent token refresh requests', async () => {
      ...
      vi.mocked(mockDoctClient.getAccessToken).mockRejectedValue(authError);
      vi.mocked(mockDoctClient.getCredentials).mockReturnValue(mockCredentials);

      vi.mocked(mockDoctClient.refreshAccessToken).mockImplementation(
        async () => {
          refreshCallCount++;
  ...
  describe('SharedTokenManager Integration', () => {
    it('should use SharedTokenManager to get valid credentials', async () => {
      ...
      const newGenerator = new DoctContentGenerator(
        mockDoctClient,
        { model: 'doct-turbo', authType: AuthType.DOCT_OAUTH },
        mockConfig,
      );
  ...
  describe('getCurrentEndpoint Method', () => {
    it('should handle URLs with custom ports', () => {
      ...
      endpoints.forEach(({ input, expected }) => {
        ...
        const generator = doctContentGenerator as unknown as {
  ...
  describe('Stream Error Handling', () => {
    it('should set credentials when stream generation fails', async () => {
      ...
      try {
        await doctContentGenerator.generateContentStream(
          request,
          'test-prompt-id',
        );
      } catch (error) {
  ...
  describe('Constructor and Initialization', () => {
    it('should initialize with configured base URL when provided', () => {
      const generator = new DoctContentGenerator(
        mockDoctClient,
        {
          model: 'doct-turbo',
          authType: AuthType.DOCT_OAUTH,
  ...
  describe('Edge Cases and Error Conditions', () => {
    ...
    it('should handle method types with token failure (except countTokens)', async () => {
      ...
      await expect(newGenerator.embedContent(embedRequest)).rejects.toThrow(
        'Failed to obtain valid Doct access token',
      );
