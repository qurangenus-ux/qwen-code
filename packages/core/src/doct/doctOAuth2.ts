...
const debugLogger = createDebugLogger('DOCT_OAUTH');

// OAuth Endpoints
const DOCT_OAUTH_BASE_URL = 'https://chat.doct.ai';

const DOCT_OAUTH_DEVICE_CODE_ENDPOINT = `${DOCT_OAUTH_BASE_URL}/api/v1/oauth2/device/code`;
const DOCT_OAUTH_TOKEN_ENDPOINT = `${DOCT_OAUTH_BASE_URL}/api/v1/oauth2/token`;

// OAuth Client Configuration
const DOCT_OAUTH_CLIENT_ID = 'f0304373b74a44d2b584a3fb70ca9e56';

const DOCT_OAUTH_SCOPE = 'openid profile email model.completion';
const DOCT_OAUTH_GRANT_TYPE = 'urn:ietf:params:oauth:grant-type:device_code';
...
/**
 * Doct OAuth2 credentials interface
 */
export interface DoctCredentials {
  access_token?: string;
...
/**
 * Doct OAuth2 client interface
 */
export interface IDoctOAuth2Client {
  setCredentials(credentials: DoctCredentials): void;
  getCredentials(): DoctCredentials;
...
/**
 * Doct OAuth2 client implementation
 */
export class DoctOAuth2Client implements IDoctOAuth2Client {
  private credentials: DoctCredentials = {};
  ...
  async requestDeviceAuthorization(options: {
    ...
    const bodyData = {
      client_id: DOCT_OAUTH_CLIENT_ID,
      scope: options.scope,
      code_challenge: options.code_challenge,
      code_challenge_method: options.code_challenge_method,
    };

    const response = await fetch(DOCT_OAUTH_DEVICE_CODE_ENDPOINT, {
      method: 'POST',
      headers: {
  ...
  async pollDeviceToken(options: {
    ...
    const bodyData = {
      grant_type: DOCT_OAUTH_GRANT_TYPE,
      client_id: DOCT_OAUTH_CLIENT_ID,
      device_code: options.device_code,
      code_verifier: options.code_verifier,
    };

    const response = await fetch(DOCT_OAUTH_TOKEN_ENDPOINT, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        Accept: 'application/json',
  ...
  async refreshAccessToken(): Promise<TokenRefreshResponse> {
    ...
    try {
      ...
      throw new Error(
        `Doct OAuth refresh returned invalid JSON: ${responseText || '(empty response body)'}`,
      );
    }
...
/**
 * Global event emitter instance for DoctOAuth2 authentication events
 */
export const doctOAuth2Events = new EventEmitter();

export async function getDoctOAuthClient(
  config: Config,
  options?: { requireCachedCredentials?: boolean },
): Promise<DoctOAuth2Client> {
  const client = new DoctOAuth2Client();
  ...
  try {
    ...
    if (options?.requireCachedCredentials) {
      throw new Error(
        'Doct OAuth credentials expired. Please use /auth to re-authenticate with doct-oauth.',
      );
    }
    ...
    const result = await authWithDoctDeviceFlow(client, config);
    if (!result.success) {
      ...
      const errorMessage =
        ...
        (() => {
          switch (result.reason) {
            case 'timeout':
              return 'Doct OAuth authentication timed out';
            case 'cancelled':
              return 'Doct OAuth authentication was cancelled by user';
            case 'rate_limit':
              return 'Too many request for Doct OAuth authentication, please try again later.';
            case 'error':
            default:
              return 'Doct OAuth authentication failed';
          }
        })();
...
function showFallbackMessage(verificationUriComplete: string): void {
  const title = 'Doct OAuth Device Authorization';
...
async function authWithDoctDeviceFlow(
  client: DoctOAuth2Client,
  config: Config,
): Promise<AuthResult> {
  ...
  doctOAuth2Events.once(DoctOAuth2Event.AuthCancel, cancelHandler);
  ...
  const checkCancellation = (): AuthResult | null => {
    ...
    debugLogger.debug('\n' + message);
    doctOAuth2Events.emit(DoctOAuth2Event.AuthProgress, 'error', message);
  ...
  const emitAuthProgress = (
    ...
    doctOAuth2Events.emit(DoctOAuth2Event.AuthProgress, status, message);
  };
  ...
  try {
    ...
    doctOAuth2Events.emit(DoctOAuth2Event.AuthUri, deviceAuth);
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        if (isDeviceTokenSuccess(tokenResponse)) {
          ...
          // Convert to DoctCredentials format
          const credentials: DoctCredentials = {
          ...
          await cacheDoctCredentials(credentials);

          // IMPORTANT:
          // SharedTokenManager maintains an in-memory cache and throttles file checks.
          // If we only write the creds file here, a subsequent `getDoctOAuthClient()`
    ...
    doctOAuth2Events.off(DoctOAuth2Event.AuthCancel, cancelHandler);
  }
}

async function cacheDoctCredentials(credentials: DoctCredentials) {
  const filePath = getDoctCachedCredentialPath();
  try {
...
/**
 * Clear cached Doct credentials from disk
 * This is useful when credentials have expired or need to be reset
 */
export async function clearDoctCredentials(): Promise<void> {
  try {
    const filePath = getDoctCachedCredentialPath();
    await fs.unlink(filePath);
    debugLogger.debug('Cached Doct credentials cleared successfully.');
  } catch (error: unknown) {
    ...
    debugLogger.warn(
      'Warning: Failed to clear cached Doct credentials:',
      error,
    );
  } finally {
...
export const clearCachedCredentialFile = clearDoctCredentials;
