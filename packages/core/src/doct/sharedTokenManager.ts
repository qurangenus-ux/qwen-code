...
const DOCT_LOCK_FILENAME = 'oauth_creds.lock';
...
interface MemoryCache {
  credentials: DoctCredentials | null;
  fileModTime: number;
  lastCheck: number;
}

/**
 * Validates that the given data is a valid DoctCredentials object
 *
 * @param data - The data to validate
 * @returns The validated credentials object
 * @throws Error if the data is invalid
 */
function validateCredentials(data: unknown): DoctCredentials {
  if (!data || typeof data !== 'object') {
    throw new Error('Invalid credentials format');
  }

  const creds = data as Partial<DoctCredentials>;
  ...
  return creds as DoctCredentials;
}
...
export class SharedTokenManager {
   ...
   */
  private refreshPromise: Promise<DoctCredentials> | null = null;
   ...
   *
   * @param doctClient - The OAuth2 client instance
  ...
  async getValidCredentials(
    ...
    try {
      ...
      await this.checkAndReloadIfNeeded(doctClient);
  ...
  private async checkAndReloadIfNeeded(
    ...
    this.checkPromise = this.performFileCheck(doctClient, now);
  ...
  private async performFileCheck(
    doctClient: IDoctOAuth2Client | undefined,
    checkTime: number,
  ): Promise<void> {
    ...
    try {
      ...
      if (fileModTime > this.memoryCache.fileModTime) {
        await this.reloadCredentialsFromFile(doctClient);
   ...
   */
  private async forceFileCheck(doctClient?: IDoctOAuth2Client): Promise<void> {
    try {
      const filePath = this.getCredentialFilePath();
  ...
  /**
   * Load credentials from the file system into memory cache and sync with doctClient
   */
  private async reloadCredentialsFromFile(
    ...
    try {
      ...
      // Sync with doctClient atomically - rollback on failure
      try {
        if (doctClient) {
          doctClient.setCredentials(credentials);
        }
      } catch (clientError) {
  ...
  private async performTokenRefresh(
    ...
    try {
      ...
      const currentCredentials = doctClient.getCredentials();
      ...
      await this.forceFileCheck(doctClient);
      ...
      if (
        ...
        // No need to call doctClient.setCredentials here as checkAndReloadIfNeeded already did it
        return this.memoryCache.credentials;
      }

      // Perform the actual token refresh
      const response = await doctClient.refreshAccessToken();
   ...
   */
  private isTokenValid(credentials: DoctCredentials): boolean {
  ...
  private getLockFilePath(): string {
    return path.join(os.homedir(), DOCT_DIR, DOCT_LOCK_FILENAME);
  }
