...
/**
 * Doct Content Generator that uses Doct OAuth tokens with automatic refresh
 */
export class DoctContentGenerator extends OpenAIContentGenerator {
  private readonly debugLogger = createDebugLogger('DOCT');
  private doctClient: IDoctOAuth2Client;
  ...
  constructor(
    ...
    // Create DashScope provider for Doct
  ...
  private async executeWithCredentialManagement<T>(
    ...
    try {
      ...
      if (this.isAuthError(error)) {
        ...
        await this.sharedManager.getValidCredentials(this.doctClient, true);
        return await attemptOperation();
      }
      throw error;
    }
