...
interface MockSharedTokenManager {
  getValidCredentials(doctClient: DoctOAuth2Client): Promise<DoctCredentials>;
...
vi.mock('./sharedTokenManager.js', () => ({
  SharedTokenManager: class {
    ...
    async getValidCredentials(
      doctClient: DoctOAuth2Client,
    ): Promise<DoctCredentials> {
      // Try to get credentials from the client first
      const clientCredentials = doctClient.getCredentials();
  ...
  describe('isErrorResponse', () => {
    ...
    it('should return false for successful responses', () => {
      const successResponse: DeviceAuthorizationResponse = {
        device_code: 'test-device-code',
        user_code: 'TEST123',
        verification_uri: 'https://chat.doct.ai/device',
        verification_uri_complete: 'https://chat.doct.ai/device?code=TEST123',
        expires_in: 1800,
      };
...
describe('DoctOAuth2Client', () => {
  let client: DoctOAuth2Client;
  let originalFetch: typeof global.fetch;

  beforeEach(() => {
    // Create client instance
    client = new DoctOAuth2Client();
    ...
    it('should refresh token if access token is expired', async () => {
      ...
      (
        client as unknown as {
          sharedManager: {
            getValidCredentials: () => Promise<DoctCredentials>;
          };
        }
      ).sharedManager = {
  ...
  describe('refreshAccessToken error handling', () => {
    ...
    it('should NOT clear credentials on malformed 200 response (e.g. proxy HTML)', async () => {
      const { CredentialsClearRequiredError } = await import('./doctOAuth2.js');
      ...
      await expect(client.refreshAccessToken()).rejects.toThrow(
        'Doct OAuth refresh returned invalid JSON:',
      );
    });
...
describe('getDoctOAuthClient', () => {
  ...
  it('should load cached credentials if available', async () => {
    ...
    const client = await import('./doctOAuth2.js').then((module) =>
      module.getDoctOAuthClient(mockConfig),
    );
  ...
  it('should handle cached credentials refresh failure', async () => {
    ...
    await expect(
      import('./doctOAuth2.js').then((module) =>
        module.getDoctOAuthClient(mockConfig),
      ),
  ...
  it('should include troubleshooting hints when device auth fetch fails', async () => {
    ...
    const emitSpy = vi.spyOn(doctOAuth2Events, 'emit');

    let thrownError: unknown;
    try {
      const { getDoctOAuthClient } = await import('./doctOAuth2.js');
      await getDoctOAuthClient(mockConfig);
    } catch (error: unknown) {
      thrownError = error;
    }
...
describe('clearDoctCredentials', () => {
  it('should successfully clear credentials file', async () => {
    const { promises: fs } = await import('node:fs');
    const { clearDoctCredentials } = await import('./doctOAuth2.js');

    vi.mocked(fs.unlink).mockResolvedValue(undefined);

    await expect(clearDoctCredentials()).resolves.not.toThrow();
...
describe('DoctOAuth2Client - Additional Error Scenarios', () => {
  let client: DoctOAuth2Client;
  let originalFetch: typeof global.fetch;
...
describe('getDoctOAuthClient - Enhanced Error Scenarios', () => {
  let mockConfig: Config;
  let originalFetch: typeof global.fetch;
...
describe('authWithDoctDeviceFlow - Comprehensive Testing', () => {
  let mockConfig: Config;
  let originalFetch: typeof global.fetch;
...
describe('Event Emitter Integration', () => {
  it('should export doctOAuth2Events as EventEmitter', async () => {
    const { doctOAuth2Events } = await import('./doctOAuth2.js');
    expect(doctOAuth2Events).toBeInstanceOf(EventEmitter);
  });

  it('should define correct event enum values', async () => {
    const { DoctOAuth2Event } = await import('./doctOAuth2.js');
    expect(DoctOAuth2Event.AuthUri).toBe('auth-uri');
    expect(DoctOAuth2Event.AuthProgress).toBe('auth-progress');
    expect(DoctOAuth2Event.AuthCancel).toBe('auth-cancel');
  });
});
...
describe('Utility Functions', () => {
  ...
  describe('getDoctCachedCredentialPath', () => {
    it('should return correct path to cached credentials', async () => {
      const os = await import('os');
      const path = await import('path');

      const expectedPath = path.join(os.homedir(), '.doct', 'oauth_creds.json');

      // Since this is a private function, we test it indirectly through clearDoctCredentials
      const { promises: fs } = await import('node:fs');
      const { clearDoctCredentials } = await import('./doctOAuth2.js');
...
describe('Credential Caching Functions', () => {
  describe('cacheDoctCredentials', () => {
    it('should create directory and write credentials to file', async () => {
      // Mock the internal cacheDoctCredentials function by creating client and calling refresh
...
describe('Enhanced Error Handling and Edge Cases', () => {
  ...
  describe('DoctOAuth2Client getAccessToken enhanced scenarios', () => {
  ...
  describe('Enhanced refreshAccessToken scenarios', () => {
    it('should call clearDoctCredentials on 400 error', async () => {
      client.setCredentials({
...
describe('SharedTokenManager Integration in DoctOAuth2Client', () => {
  let client: DoctOAuth2Client;
  ...
  it('should handle TokenManagerError types correctly in getDoctOAuthClient', async () => {
    const mockConfig = {
    ...
    for (const errorInfo of tokenErrors) {
      ...
      try {
        await import('./doctOAuth2.js').then((module) =>
          module.getDoctOAuthClient(mockConfig),
        );
      } catch {
...
describe('Constants and Configuration', () => {
  it('should have correct OAuth endpoints', async () => {
    ...
    expect(url).toBe('https://chat.doct.ai/api/v1/oauth2/device/code');
  });
