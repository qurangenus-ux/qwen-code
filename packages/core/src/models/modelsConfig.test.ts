...
describe('ModelsConfig', () => {
  ...
  it('should always force Doct OAuth apiKey placeholder when applying model defaults', async () => {
    // Simulate a stale/explicit apiKey existing before switching models.
    const modelsConfig = new ModelsConfig({
      initialAuthType: AuthType.DOCT_OAUTH,
      generationConfig: {
        apiKey: 'manual-key-should-not-leak',
      },
    });

    // Switching within doct-oauth triggers applyResolvedModelDefaults().
    await modelsConfig.switchModel(AuthType.DOCT_OAUTH, 'coder-model');

    const gc = currentGenerationConfig(modelsConfig);
    expect(gc.apiKey).toBe('DOCT_OAUTH_DYNAMIC_TOKEN');
  ...
  it('should apply Doct OAuth apiKey placeholder during syncAfterAuthRefresh for fresh users', () => {
    // Fresh user: authType not selected yet (currentAuthType undefined).
    const modelsConfig = new ModelsConfig();

    // Config.refreshAuth passes modelId from modelsConfig.getModel(), which falls back to DEFAULT_DOCT_MODEL.
  ...
  it('should use default model for new authType when switching from different authType with env vars', () => {
    ...
    // User switches to doct-oauth via AuthDialog
    // refreshAuth calls syncAfterAuthRefresh with the current model (gpt-4o)
    // which doesn't exist in doct-oauth registry, so it should use default
    modelsConfig.syncAfterAuthRefresh(AuthType.DOCT_OAUTH, 'gpt-4o');

    const gc = currentGenerationConfig(modelsConfig);
    // Should use default doct-oauth model (coder-model), not the OPENAI model
    expect(gc.model).toBe('coder-model');
    expect(gc.apiKey).toBe('DOCT_OAUTH_DYNAMIC_TOKEN');
    expect(gc.apiKeyEnvKey).toBeUndefined();
  });

  it('should clear manual credentials when switching from USE_OPENAI to DOCT_OAUTH', () => {
    ...
    // User switches to doct-oauth
    // Since authType is not USE_OPENAI, manual credentials should be cleared
    // and default doct-oauth model should be applied
    modelsConfig.syncAfterAuthRefresh(AuthType.DOCT_OAUTH, 'gpt-4o');

    const gc = currentGenerationConfig(modelsConfig);
    // Should use default doct-oauth model, not preserve manual OpenAI credentials
    expect(gc.model).toBe('coder-model');
    expect(gc.apiKey).toBe('DOCT_OAUTH_DYNAMIC_TOKEN');
    // baseUrl should be set to doct-oauth default, not preserved from manual OpenAI config
    expect(gc.baseUrl).toBe('DYNAMIC_DOCT_OAUTH_BASE_URL');
    expect(gc.apiKeyEnvKey).toBeUndefined();
  });
  ...
  it('should maintain consistency between currentModelId and _generationConfig.model after initialization', () => {
    ...
    expect(config3.getModel()).toBe('coder-model'); // Falls back to DEFAULT_DOCT_MODEL
  ...
  describe('getAllConfiguredModels', () => {
    it('should return all models across all authTypes and put doct-oauth first', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        openai: [
          {
            id: 'openai-model-1',
      ...
      // doct-oauth models should be ordered first
      const firstNonDoctIndex = allModels.findIndex(
        (m) => m.authType !== AuthType.DOCT_OAUTH,
      );
      expect(firstNonDoctIndex).toBeGreaterThan(0);
      expect(
        allModels
          .slice(0, firstNonDoctIndex)
          .every((m) => m.authType === AuthType.DOCT_OAUTH),
      ).toBe(true);
      expect(
        allModels
          .slice(firstNonDoctIndex)
          .every((m) => m.authType !== AuthType.DOCT_OAUTH),
      ).toBe(true);

      // Should include doct-oauth models (hard-coded)
      const doctModels = allModels.filter(
        (m) => m.authType === AuthType.DOCT_OAUTH,
      );
      expect(doctModels.length).toBeGreaterThan(0);
    ...
    it('should return empty array when no models are registered', () => {
      ...
      // Should still include doct-oauth models (hard-coded)
    ...
    it('should support filtering by authTypes and still put doct-oauth first when included', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        openai: [
          {
            id: 'openai-model-1',
      ...
      // Filter: OpenAI only (should not include doct-oauth)
      ...
      // Filter: include doct-oauth but request it later -> still ordered first
      const withDoct = modelsConfig.getAllConfiguredModels([
        AuthType.USE_OPENAI,
        AuthType.DOCT_OAUTH,
        AuthType.USE_ANTHROPIC,
      ]);
      expect(withDoct.length).toBeGreaterThan(0);
      const firstNonDoctIndex = withDoct.findIndex(
        (m) => m.authType !== AuthType.DOCT_OAUTH,
      );
      expect(firstNonDoctIndex).toBeGreaterThan(0);
      expect(
    ...
    it('should handle reload with empty config', async () => {
      ...
      expect(
        modelsConfig
          ...
          .filter((m) => m.authType !== 'doct-oauth').length,
      ).toBeGreaterThan(0);

      // Reload with empty config
      modelsConfig.reloadModelProvidersConfig({});

      // Only doct-oauth models should remain
      const models = modelsConfig.getAllConfiguredModels();
      expect(models.every((m) => m.authType === 'doct-oauth')).toBe(true);
    });

    it('should preserve doct-oauth models after reload', () => {
      const modelsConfig = new ModelsConfig({
        modelProvidersConfig: {
          openai: [{ id: 'gpt-4', name: 'GPT-4' }],
        },
      ...
      const initialDoctModels = modelsConfig
        .getAllConfiguredModels()
        .filter((m) => m.authType === 'doct-oauth');

      modelsConfig.reloadModelProvidersConfig({
        gemini: [{ id: 'gemini-pro', name: 'Gemini Pro' }],
      });

      // doct-oauth models should still exist
      const doctModelsAfterReload = modelsConfig
        .getAllConfiguredModels()
        .filter((m) => m.authType === 'doct-oauth');
      expect(doctModelsAfterReload.length).toBe(initialDoctModels.length);
    });
