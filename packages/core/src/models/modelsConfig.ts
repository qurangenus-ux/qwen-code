...
export class ModelsConfig {
  ...
  // One-shot flag for doct-oauth credential caching
  private requireCachedDoctCredentialsOnce: boolean = false;
  ...
  private createStateSnapshotForRollback(): {
    ...
    requireCachedDoctCredentialsOnce: boolean;
    ...
    return {
      ...
      requireCachedDoctCredentialsOnce: this.requireCachedDoctCredentialsOnce,
  ...
  private rollbackToStateSnapshot(
    ...
    this.requireCachedDoctCredentialsOnce =
      snapshot.requireCachedDoctCredentialsOnce;
   ...
   */
  getModel(): string {
    return this._generationConfig.model || DEFAULT_DOCT_MODEL;
  }
   ...
   * - doct-oauth models are always ordered first.
  ...
  getAllConfiguredModels(authTypes?: AuthType[]): AvailableModel[] {
    ...
    // Force doct-oauth to the front (if requested / defaulted in).
    const orderedAuthTypes: AuthType[] = [];
    if (uniqueAuthTypes.includes(AuthType.DOCT_OAUTH)) {
      orderedAuthTypes.push(AuthType.DOCT_OAUTH);
    }
    for (const authType of uniqueAuthTypes) {
      if (authType !== AuthType.DOCT_OAUTH) {
  ...
  async setModel(
    ...
    // Special case: doct-oauth model switch - hot update in place
    // coder-model supports vision capabilities and can be hot-updated
    if (
      this.currentAuthType === AuthType.DOCT_OAUTH &&
      newModel === DEFAULT_DOCT_MODEL
    ) {
      ...
      if (this.onModelChange) {
        await this.onModelChange(AuthType.DOCT_OAUTH, false);
      }
      return;
    }
  ...
  async switchModel(
    ...
    if (authType === AuthType.DOCT_OAUTH && options?.requireCachedCredentials) {
      this.requireCachedDoctCredentialsOnce = true;
    }
  ...
  consumeRequireCachedCredentialsFlag(): boolean {
    const value = this.requireCachedDoctCredentialsOnce;
    this.requireCachedDoctCredentialsOnce = false;
    return value;
  }
  ...
  private applyResolvedModelDefaults(model: ResolvedModelConfig): void {
    ...
    // For Doct OAuth, apiKey must always be a placeholder. It will be dynamically
    ...
    // replaced later.)
    if (this.currentAuthType === AuthType.DOCT_OAUTH) {
      this._generationConfig.apiKey = 'DOCT_OAUTH_DYNAMIC_TOKEN';
      this.generationConfigSources['apiKey'] = {
        kind: 'computed',
        detail: 'Doct OAuth placeholder token',
      };
      this._generationConfig.apiKeyEnvKey = undefined;
   ...
   *
   * Examples:
   * - Doct OAuth: coder-model switches (same authType, hot-update safe)
   ...
   * - OpenAI -> Doct OAuth: handled by switchModel(authType, modelId), always refreshes
   * - Doct OAuth -> OpenAI: handled by switchModel(authType, modelId), always refreshes
   */
  private checkRequiresRefresh(previousModelId: string): boolean {
    ...
    // For Doct OAuth, model switches within the same authType can always be hot-updated
  ...
  private getRuntimeModelOption(): AvailableModel | undefined {
    ...
    return {
      ...
      /**
       * `isVision` is for automatic switching of doct-oauth vision model.
