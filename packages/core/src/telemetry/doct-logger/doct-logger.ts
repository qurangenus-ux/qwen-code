...
export class DoctLogger {
  private static instance: DoctLogger;
  ...
  private constructor(config: Config) {
    this.config = config;
    this.debugLogger = createDebugLogger('DOCT_LOGGER');
  ...
  static getInstance(config?: Config): DoctLogger | undefined {
    if (config === undefined || !config?.getUsageStatisticsEnabled())
      return undefined;
    if (!DoctLogger.instance) {
      DoctLogger.instance = new DoctLogger(config);
    }

    return DoctLogger.instance;
  }
    ...
    try {
      ...
      if (wasAtCapacity) {
        this.debugLogger.debug(
          `DoctLogger: Dropped old event to prevent memory leak (queue size: ${this.events.size})`,
        );
      }
    } catch (error) {
      this.debugLogger.error('DoctLogger: Failed to enqueue log event.', error);
    }
  }
  ...
  readSourceInfo(): string {
    try {
      const sourceJsonPath = path.join(os.homedir(), '.doct', 'source.json');
  ...
  async flushToRum(): Promise<LogResponse> {
    if (this.isFlushInProgress) {
      this.debugLogger.debug(
        'DoctLogger: Flush already in progress, marking pending flush.',
      );
  ...
  private requeueFailedEvents(eventsToSend: RumEvent[]): void {
    ...
    this.debugLogger.debug(
      `DoctLogger: Re-queued ${numEventsToRequeue} events for retry (queue size: ${this.events.size})`,
    );
  }
}
