...
import { DoctLogger, TEST_ONLY } from './doct-logger.js';
...
const makeFakeConfig = (overrides: Partial<Config> = {}): Config => {
  const defaults = {
    ...
    getAuthType: () => AuthType.DOCT_OAUTH,
    getMcpServers: () => ({}),
    getModel: () => 'test-model',
...
describe('DoctLogger', () => {
  ...
  beforeEach(() => {
    ...
    (DoctLogger as any).instance = undefined;
  });
  ...
  describe('getInstance', () => {
    it('returns undefined when usage statistics are disabled', () => {
      ...
      const logger = DoctLogger.getInstance(config);
      expect(logger).toBeUndefined();
    });

    it('returns an instance when usage statistics are enabled', () => {
      const logger = DoctLogger.getInstance(mockConfig);
      expect(logger).toBeInstanceOf(DoctLogger);
    });

    it('is a singleton', () => {
      const logger1 = DoctLogger.getInstance(mockConfig);
      const logger2 = DoctLogger.getInstance(mockConfig);
  ...
  describe('logHookCallEvent', () => {
    ...
    it('should log a failed hook call event with error when telemetry log prompts enabled', () => {
      ...
      const logger = DoctLogger.getInstance(configWithLogPrompts)!;
      const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
    ...
    it('should not include error when telemetry log prompts disabled', () => {
      ...
      (DoctLogger as unknown as { instance: undefined }).instance = undefined;
      const logger = DoctLogger.getInstance(configWithoutLogPrompts)!;
      const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
    ...
    it('should sanitize hook name to remove sensitive information', () => {
      ...
      const event = new HookCallEvent(
        'PreToolUse',
        'command',
        '/home/user/.doct/hooks/check-secrets.sh --api-key=secret123',
        { tool_name: 'read_file' },
        100,
        true,
      );
