...
import { DoctLogger } from './doct-logger/doct-logger.js';
...
export function logStartSession(
  ...
  DoctLogger.getInstance(config)?.logStartSessionEvent(event);
...
export function logUserPrompt(config: Config, event: UserPromptEvent): void {
  DoctLogger.getInstance(config)?.logNewPromptEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logUserRetry(config: Config, event: UserRetryEvent): void {
  DoctLogger.getInstance(config)?.logRetryEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logToolCall(config: Config, event: ToolCallEvent): void {
  ...
  DoctLogger.getInstance(config)?.logToolCallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logToolOutputTruncated(
  ...
  DoctLogger.getInstance(config)?.logToolOutputTruncatedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logFileOperation(
  ...
  DoctLogger.getInstance(config)?.logFileOperationEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiRequest(config: Config, event: ApiRequestEvent): void {
  // DoctLogger.getInstance(config)?.logApiRequestEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logFlashFallback(
  ...
  DoctLogger.getInstance(config)?.logFlashFallbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logRipgrepFallback(
  ...
  DoctLogger.getInstance(config)?.logRipgrepFallbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiError(config: Config, event: ApiErrorEvent): void {
  ...
  DoctLogger.getInstance(config)?.logApiErrorEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiCancel(config: Config, event: ApiCancelEvent): void {
  ...
  uiTelemetryService.addEvent(uiEvent);
  DoctLogger.getInstance(config)?.logApiCancelEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiResponse(config: Config, event: ApiResponseEvent): void {
  ...
  DoctLogger.getInstance(config)?.logApiResponseEvent(event);
  if (!isTelemetrySdkInitialized()) return;
  const attributes: LogAttributes = {
    ...getCommonAttributes(config),
    ...event,
...
export function logLoopDetected(
  ...
  DoctLogger.getInstance(config)?.logLoopDetectedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logLoopDetectionDisabled(
  ...
  DoctLogger.getInstance(config)?.logLoopDetectionDisabledEvent();
}
...
export function logNextSpeakerCheck(
  ...
  DoctLogger.getInstance(config)?.logNextSpeakerCheck(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logSlashCommand(
  ...
  DoctLogger.getInstance(config)?.logSlashCommandEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logIdeConnection(
  ...
  DoctLogger.getInstance(config)?.logIdeConnectionEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logConversationFinishedEvent(
  ...
  DoctLogger.getInstance(config)?.logConversationFinishedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logChatCompression(
  ...
  DoctLogger.getInstance(config)?.logChatCompressionEvent(event);
...
export function logKittySequenceOverflow(
  ...
  DoctLogger.getInstance(config)?.logKittySequenceOverflowEvent(event);
  if (!isTelemetrySdkInitialized()) return;
  const attributes: LogAttributes = {
    ...getCommonAttributes(config),
    ...event,
...
export function logMalformedJsonResponse(
  ...
  DoctLogger.getInstance(config)?.logMalformedJsonResponseEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logInvalidChunk(
  ...
  DoctLogger.getInstance(config)?.logInvalidChunkEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logContentRetry(
  ...
  DoctLogger.getInstance(config)?.logContentRetryEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logContentRetryFailure(
  ...
  DoctLogger.getInstance(config)?.logContentRetryFailureEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logSubagentExecution(
  ...
  DoctLogger.getInstance(config)?.logSubagentExecutionEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logModelSlashCommand(
  ...
  DoctLogger.getInstance(config)?.logModelSlashCommandEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logHookCall(config: Config, event: HookCallEvent): void {
  // Log to DoctLogger for RUM telemetry only
  DoctLogger.getInstance(config)?.logHookCallEvent(event);
}
...
export function logExtensionInstallEvent(
  ...
  DoctLogger.getInstance(config)?.logExtensionInstallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logExtensionUninstall(
  ...
  DoctLogger.getInstance(config)?.logExtensionUninstallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export async function logExtensionUpdateEvent(
  ...
  DoctLogger.getInstance(config)?.logExtensionUpdateEvent(event);
...
export function logExtensionEnable(
  ...
  DoctLogger.getInstance(config)?.logExtensionEnableEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logExtensionDisable(
  ...
  DoctLogger.getInstance(config)?.logExtensionDisableEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logAuth(config: Config, event: AuthEvent): void {
  DoctLogger.getInstance(config)?.logAuthEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logUserFeedback(
  ...
  uiTelemetryService.addEvent(uiEvent);
  config.getChatRecordingService()?.recordUiTelemetryEvent(uiEvent);
  DoctLogger.getInstance(config)?.logUserFeedbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaSessionStarted(
  ...
  DoctLogger.getInstance(config)?.logArenaSessionStartedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaAgentCompleted(
  ...
  DoctLogger.getInstance(config)?.logArenaAgentCompletedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaSessionEnded(
  ...
  DoctLogger.getInstance(config)?.logArenaSessionEndedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
