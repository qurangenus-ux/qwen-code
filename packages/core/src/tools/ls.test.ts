...
describe('LSTool', () => {
  ...
  beforeEach(async () => {
    ...
    const userSkillsBase = path.join(os.homedir(), '.doct', 'skills');
  ...
  describe('execute', () => {
    ...
    it('should respect doctignore patterns', async () => {
      await fs.writeFile(path.join(tempRootDir, 'file1.txt'), 'content1');
      await fs.writeFile(path.join(tempRootDir, 'file2.log'), 'content1');
      await fs.writeFile(path.join(tempRootDir, '.doctignore'), '*.log');
      const invocation = lsTool.build({ path: tempRootDir });
      const result = await invocation.execute(abortSignal);

      expect(result.llmContent).toContain('file1.txt');
      expect(result.llmContent).not.toContain('file2.log');
      expect(result.returnDisplay).toBe('Listed 2 item(s) (1 doct-ignored)');
    });
