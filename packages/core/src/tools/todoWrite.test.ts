...
describe('TodoWriteTool – runtime output directory', () => {
  ...
  it('should write todos to env var dir when DOCT_RUNTIME_DIR is set', async () => {
    const envRuntimeDir = path.resolve('env', 'runtime');
    process.env['DOCT_RUNTIME_DIR'] = envRuntimeDir;
  ...
  it('should use default ~/.doct path when no custom dir is configured', async () => {
    const params: TodoWriteParams = {
      todos: [{ id: '1', content: 'Task 1', status: 'pending' }],
    };
    ...
    const writePath = mockFs.writeFile.mock.calls[0]?.[0] as string;
    expect(writePath).toContain(path.join('.doct', 'todos'));
  });
