...
export interface LSToolParams {
  ...
  /**
   * Whether to respect .gitignore and .doctignore patterns (optional, defaults to true)
   */
  file_filtering_options?: {
    respect_git_ignore?: boolean;
    respect_doct_ignore?: boolean;
  };
}
...
class LSToolInvocation extends BaseToolInvocation<LSToolParams, ToolResult> {
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    try {
      ...
      const { filteredPaths, gitIgnoredCount, doctIgnoredCount } =
        fileDiscovery.filterFilesWithReport(relativePaths, {
          ...
          respectDoctIgnore:
            this.params.file_filtering_options?.respect_doct_ignore ??
            this.config.getFileFilteringOptions().respectDoctIgnore ??
            DEFAULT_FILE_FILTERING_OPTIONS.respectDoctIgnore,
        });
      ...
      if (doctIgnoredCount > 0) {
        ignoredMessages.push(`${doctIgnoredCount} doct-ignored`);
      }
    ...
    super(
      ...
      Kind.Search,
      {
        properties: {
          ...
          file_filtering_options: {
            description:
              'Optional: Whether to respect ignore patterns from .gitignore or .doctignore',
            type: 'object',
            properties: {
              respect_git_ignore: {
                description:
              ...
              respect_doct_ignore: {
                description:
                  'Optional: Whether to respect .doctignore patterns when listing files. Defaults to true.',
                type: 'boolean',
              },
            },
          },
