...
const MEMORY_SECTION_HEADER = '## Doct Added Memories';
...
describe('MemoryTool', () => {
  ...
  describe('performAddMemoryEntry (static method)', () => {
    ...
    beforeEach(() => {
      testFilePath = path.join(os.homedir(), '.doct', DEFAULT_CONTEXT_FILENAME);
    });
  ...
  describe('getDefaultPermission and getConfirmationDetails', () => {
    ...
    it('should return confirmation details for global scope', async () => {
      ...
      if (result.type === 'edit') {
        const expectedPath = path.join('~', '.doct', 'DOCT.md');
        expect(result.title).toBe(
          `Confirm Memory Save: ${expectedPath} (global)`,
        );
        expect(result.fileName).toContain(path.join('mock', 'home', '.doct'));
        expect(result.fileName).toContain('DOCT.md');
        expect(result.fileDiff).toContain('Index: DOCT.md');
        expect(result.fileDiff).toContain('+## Doct Added Memories');
        expect(result.fileDiff).toContain('+- Test fact');
        expect(result.originalContent).toBe('');
        expect(result.newContent).toContain('## Doct Added Memories');
    ...
    it('should return confirmation details for project scope', async () => {
      ...
      if (result.type === 'edit') {
        const expectedPath = path.join(process.cwd(), 'DOCT.md');
        expect(result.title).toBe(
    ...
    it('should prompt for scope selection when scope is not specified', async () => {
      ...
      if (result.type === 'edit') {
        ...
        expect(result.fileName).toBe('DOCT.md');
        expect(result.fileDiff).toContain('Test fact');
        expect(result.fileDiff).toContain('--- DOCT.md');
        expect(result.fileDiff).toContain('+++ DOCT.md');
        expect(result.fileDiff).toContain('+- Test fact');
    ...
    it('should show correct file paths in scope selection prompt', async () => {
      ...
      if (result.type === 'edit') {
        const globalPath = path.join('~', '.doct', 'DOCT.md');
        const projectPath = path.join(process.cwd(), 'DOCT.md');
