...
describe('RipGrepTool', () => {
  ...
  describe('execute', () => {
    ...
    it('should pass .doctignore to ripgrep when respected', async () => {
      await fs.writeFile(
    ...
    it('should include .doctignore matches when disabled in config', async () => {
      await fs.writeFile(path.join(tempRootDir, '.doctignore'), 'kept.txt\n');
  ...
  describe('multi-directory workspace', () => {
    ...
    it('should load .doctignore from each workspace directory', async () => {
      const secondDir = await fs.mkdtemp(
        path.join(os.tmpdir(), 'grep-tool-second-'),
      );
      await fs.writeFile(path.join(secondDir, '.doctignore'), 'ignored.txt\n');
      await fs.writeFile(
        path.join(tempRootDir, '.doctignore'),
        'other-ignored.txt\n',
      );
      ...
      // Verify both .doctignore files were passed
      ...
      expect(ignoreFileArgs).toContain(path.join(tempRootDir, '.doctignore'));
      expect(ignoreFileArgs).toContain(path.join(secondDir, '.doctignore'));
