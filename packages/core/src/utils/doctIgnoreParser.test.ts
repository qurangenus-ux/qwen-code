...
import { DoctIgnoreParser } from './doctIgnoreParser.js';
import * as fs from 'node:fs/promises';
import * as path from 'node:path';
import * as os from 'node:os';

describe('DoctIgnoreParser', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'doctignore-test-'));
  });
  ...
  describe('when .doctignore exists', () => {
    beforeEach(async () => {
    ...
    it('should ignore files specified in .doctignore', () => {
      const parser = new DoctIgnoreParser(projectRoot);
  ...
  describe('when .doctignore does not exist', () => {
