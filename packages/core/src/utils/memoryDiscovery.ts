...
async function getGeminiMdFilePathsInternalForEachDir(
  ...
  for (const geminiMdFilename of geminiMdFilenames) {
    ...
    if (isHomeDirectory) {
      // For home directory, only check for DOCT.md directly in the home directory
      ...
      while (currentDir && currentDir !== path.dirname(currentDir)) {
        if (currentDir === path.join(resolvedHome, DOCT_DIR)) {
          break;
        }
...
/**
 * Loads hierarchical DOCT.md files and concatenates their content.
...
export async function loadServerHierarchicalMemory(
  ...
  if (filePaths.length === 0) {
    logger.debug('No DOCT.md files found in hierarchy.');
  ...
  // Only count files that match configured memory filenames (e.g., DOCT.md),
