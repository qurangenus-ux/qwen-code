...
export function loadIgnoreRules(options: LoadIgnoreRulesOptions): Ignore {
  ...
  if (options.useDoctignore) {
    const doctignorePath = path.join(options.projectRoot, '.doctignore');
    if (fs.existsSync(doctignorePath)) {
      ignorer.add(fs.readFileSync(doctignorePath, 'utf8'));
    }
  }
