...
export const DOCT_DIR = '.doct';
export const GOOGLE_ACCOUNTS_FILENAME = 'google_accounts.json';
export const OAUTH_FILE = 'oauth_creds.json';
export const SKILL_PROVIDER_CONFIG_DIRS = ['.doct', '.agents'];
...
export class Storage {
   ...
   * When null, falls back to getGlobalDoctDir().
   */
   ...
   * Pass null/undefined/empty string to reset to default (getGlobalDoctDir()).
   ...
   *              Pass the project root so that relative values like ".doct" resolve
   ...
   *
   * Priority: DOCT_RUNTIME_DIR/DOCT_RUNTIME_DIR env var > setRuntimeBaseDir() value > getGlobalDoctDir()
   * @returns Absolute path to the runtime output base directory
   */
  static getRuntimeBaseDir(): string {
    const envDir =
      process.env['DOCT_RUNTIME_DIR'] ?? process.env['DOCT_RUNTIME_DIR'];
    if (envDir) {
      return (
        Storage.resolveRuntimeBaseDir(envDir) ?? Storage.getGlobalDoctDir()
      );
    }

    const contextualDir = Storage.runtimeBaseDirContext.getStore();
    if (contextualDir !== undefined) {
      return contextualDir ?? Storage.getGlobalDoctDir();
    }
    if (Storage.runtimeBaseDir) {
      return Storage.runtimeBaseDir;
    }
    return Storage.getGlobalDoctDir();
  }

  static getGlobalDoctDir(): string {
    const homeDir = os.homedir();
    if (!homeDir) {
      return path.join(os.tmpdir(), '.doct');
    }
    return path.join(homeDir, DOCT_DIR);
  }

  static getMcpOAuthTokensPath(): string {
    return path.join(Storage.getGlobalDoctDir(), 'mcp-oauth-tokens.json');
  }

  static getGlobalSettingsPath(): string {
    return path.join(Storage.getGlobalDoctDir(), 'settings.json');
  }

  static getInstallationIdPath(): string {
    return path.join(Storage.getGlobalDoctDir(), 'installation_id');
  }

  static getGoogleAccountsPath(): string {
    return path.join(Storage.getGlobalDoctDir(), GOOGLE_ACCOUNTS_FILENAME);
  }

  static getUserCommandsDir(): string {
    return path.join(Storage.getGlobalDoctDir(), 'commands');
  }

  static getGlobalMemoryFilePath(): string {
    return path.join(Storage.getGlobalDoctDir(), 'memory.md');
  }
  ...
  static getPlansDir(): string {
    return path.join(Storage.getGlobalDoctDir(), PLANS_DIR_NAME);
  }
  ...
  static getGlobalBinDir(): string {
    return path.join(Storage.getGlobalDoctDir(), BIN_DIR_NAME);
  }

  static getGlobalArenaDir(): string {
    return path.join(Storage.getGlobalDoctDir(), ARENA_DIR_NAME);
  }

  getDoctDir(): string {
    return path.join(this.targetDir, DOCT_DIR);
  }
  ...
  static getOAuthCredsPath(): string {
    return path.join(Storage.getGlobalDoctDir(), OAUTH_FILE);
  }
  ...
  getWorkspaceSettingsPath(): string {
    return path.join(this.getDoctDir(), 'settings.json');
  }

  getProjectCommandsDir(): string {
    return path.join(this.getDoctDir(), 'commands');
  }
  ...
  getExtensionsDir(): string {
    return path.join(this.getDoctDir(), 'extensions');
  }

  getExtensionsConfigPath(): string {
    return path.join(this.getExtensionsDir(), 'doct-extension.json');
  }
  ...
  /**
   * Returns the user-level extensions directory (~/.doct/extensions/).
   * Extensions installed at user scope are stored here, as opposed to
   * project-level extensions which live in <project>/.doct/extensions/.
   */
  static getUserExtensionsDir(): string {
    return path.join(Storage.getGlobalDoctDir(), 'extensions');
  }
