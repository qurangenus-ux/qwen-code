...
export class ArenaManager {
  ...
  constructor(config: Config, callbacks: ArenaCallbacks = {}) {
    ...
    // Use the user-configured base dir, or default to ~/.doct/arena.
    this.arenaBaseDir =
      arenaSettings?.worktreeBaseDir ??
      path.join(Storage.getGlobalDoctDir(), 'arena');
  ...
  private buildAgentSpawnConfig(agent: ArenaAgentState): AgentSpawnConfig {
    ...
    if (model.apiKey) {
      env['DOCT_API_KEY'] = model.apiKey;
    }
    if (model.baseUrl) {
      env['DOCT_BASE_URL'] = model.baseUrl;
    }
   ...
   * `~/.doct/worktrees/<sessionId>/`.  The directory contains:
