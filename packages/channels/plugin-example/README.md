# @doct-code/channel-plugin-example

A reference channel plugin for Doct Code. It connects to a WebSocket server and routes messages through the full channel pipeline (access control, session routing, agent bridge).
...
```bash
npm install @doct-code/channel-plugin-example
```

### 2. Link it as a Doct Code extension

The package ships a `doct-extension.json` manifest, so it works as an extension out of the box:

```bash
doct extensions link ./node_modules/@doct-code/channel-plugin-example
```

### 3. Configure the channel

Add a channel entry to `~/.doct/settings.json`:
...
```bash
HTTP_PORT=8080 WS_PORT=8081 npx doct-channel-plugin-example-server
```
...
```bash
doct channel start my-plugin-test
```
...
4. Add a `doct-extension.json` manifest
