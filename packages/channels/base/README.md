# @doct-code/channel-base

Base infrastructure for building Doct Code channel adapters. Provides the abstract base class, access control, session routing, and the ACP bridge that communicates with the agent.
...
```bash
npm install @doct-code/channel-base
```
...
```typescript
import type { ChannelPlugin } from '@doct-code/channel-base';
...
For a complete working example, see [`@doct-code/channel-plugin-example`](../plugin-example/).
...
| `AcpBridge`     | Spawns and communicates with the `doct-code --acp` agent process |
...
Manages the `doct-code --acp` child process and ACP sessions.
...
Persists pairing state to `~/.doct/channels/{channelName}-pairing.json` and `{channelName}-allowlist.json`.
...
- [`@doct-code/channel-plugin-example`](../plugin-example/) — working reference implementation
