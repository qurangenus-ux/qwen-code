...
const USER_GEMINI_DIR = path.join(os.homedir(), '.doct');
// Project-level .gemini directory in the workspace
const WORKSPACE_DOCT_DIR = path.join(projectRoot, '.doct');

// Telemetry artifacts are stored in a hashed directory under the user's ~/.doct/tmp
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `doct-code-releases-${Date.now()}.json`,
  );
  try {
    const result = spawnSync(
      'curl',
      ['-sL', '-H', 'User-Agent: doct-code-dev-script', '-o', tmpFile, url],
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'doct-code-telemetry-'));
