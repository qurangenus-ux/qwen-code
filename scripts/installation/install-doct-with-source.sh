...
# Doct Code Installation Script
# This script installs Node.js (via NVM) and Doct Code CLI
# Supports Linux and macOS
#
# Usage: install-doct-with-source.sh --source [github|npm|internal|local-build]
#        install-doct-with-source.sh -s [github|npm|internal|local-build]
...
if [ -z "${BASH_VERSION}" ] && [ -z "${__DOCT_INSTALL_REEXEC:-}" ]; then
    # Check if we're in a git hook environment
    case "${0}" in
        *.git/hooks/*) export __DOCT_IN_GIT_HOOK=1 ;;
    esac
    if [ -n "${GIT_DIR:-}" ]; then
        export __DOCT_IN_GIT_HOOK=1
    fi

    # Try to find bash
    if command -v bash >/dev/null 2>&1; then
        export __DOCT_INSTALL_REEXEC=1
...
# ============================================
echo "=========================================="
echo "   Doct Code Installation Script"
echo "=========================================="
echo ""
...
install_nvm() {
    ...
    if "${DOWNLOAD_CMD}" "${DOWNLOAD_ARGS}" "https://doct-code-assets.oss-cn-hangzhou.aliyuncs.com/installation/install_nvm.sh" > "${NVM_INSTALL_TEMP}"; then
    ...
    # Check if profile file is writable
        ...
        {
            echo ""
            echo "# NVM configuration (added by Doct Code installer)"
...
fix_npm_permissions() {
    ...
    if [[ "${use_user_dir}" == true ]]; then
        ...
        if [[ -n "${PROFILE_FILE}" ]] && ! grep -q '.npm-global/bin' "${PROFILE_FILE}" 2>/dev/null; then
            {
                echo ""
                echo "# NPM global bin (added by Doct Code installer)"
...
install_doct_code() {
    ...
    if command_exists doct; then
        local DOCT_VERSION
        DOCT_VERSION=$(doct --version 2>/dev/null || echo "unknown")
        log_success "Doct Code is already installed: ${DOCT_VERSION}"
    ...
    # Install Doct Code
    log_info "Installing Doct Code..."
    if npm install -g @doct-code/doct-code@latest --registry https://registry.npmmirror.com; then
        log_success "Doct Code installed successfully!"

        # Verify installation
        if command_exists doct; then
            local doct_version
            doct_version=$(doct --version 2>/dev/null) || doct_version="unknown"
            log_info "Doct Code version: ${doct_version}"
        fi
    else
        log_error "Failed to install Doct Code!"
...
create_source_json() {
    ...
    cat > "${DOCT_DIR}/source.json" <<EOF
{
  "source": "${ESCAPED_SOURCE}"
}
EOF

    log_success "Installation source saved to ~/.doct/source.json"
}
...
main() {
    ...
    # Check if doct is immediately available
    if command_exists doct; then
        log_success "Doct Code is ready to use!"
        echo ""
        echo "You can now run: doct"
        echo ""
        # Auto-start doct
        log_info "Starting Doct Code..."
        echo ""
        exec doct
    else
        log_warning "Doct Code command not found in current session"
        echo ""
        echo "To use Doct Code immediately without restarting your terminal,"
        echo "run the following command in your current shell:"
        echo "  eval \$(${0} --print-env)"
        echo ""
        log_info "Or simply restart your terminal, then run: doct"
    fi
}
