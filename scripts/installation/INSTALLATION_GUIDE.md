# Installation Guide for Doct Code with Source Tracking

This guide describes how to install Node.js and Doct Code with source information tracking.

## Overview

The installation scripts automate the process of installing Node.js (if not present or below version 20) and Doct Code, while capturing and storing the installation source information for analytics and tracking purposes.

## Installation Scripts

We provide platform-specific installation scripts:

- **Linux/macOS**: `install-doct-with-source.sh`
- **Windows**: `install-doct-with-source.bat`

## Linux/macOS Installation

### Script: install-doct-with-source.sh

#### Features:

- Checks for existing Node.js installation and version
- Installs Node.js 20+ if needed using NVM
- Installs Doct Code globally with source information
- Stores the source information in `~/.doct/source.json`

#### Usage:

```bash
# Install with a specific source
sh install-doct-with-source.sh --source github

# Install with internal source
sh install-doct-with-source.sh -s internal

# Show help
sh install-doct-with-source.sh --help
```
...
1. The script accepts a `--source` parameter to specify where Doct Code is being installed from
2. It installs Node.js if needed
3. It installs Doct Code globally
4. It creates `~/.doct/source.json` with the specified source information
...
This is required to load the newly installed Node.js and Doct Code into your PATH.
...
### Script: install-doct-with-source.bat
...
- Installs Doct Code globally with source information
- Stores the source information in `%USERPROFILE%\.doct\source.json`

#### Prerequisites:

- **PowerShell (Administrator)**: The script must be run in PowerShell with Administrator privileges
- Internet connection for downloading Node.js and Doct Code
...
./install-doct-with-source.bat --source github

# Install with short parameter
./install-doct-with-source.bat -s internal

# Use default source (unknown)
./install-doct-with-source.bat
```
...
1. The script accepts a `--source` or `-s` parameter to specify where Doct Code is being installed from
...
4. It installs Doct Code globally using npm
5. It creates `%USERPROFILE%\.doct\source.json` with the specified source information
...
This feature implements the ability to capture and store the installation source of the Doct Code package. The source information is used for analytics and tracking purposes.

### Storage Location

The installation source is stored in a separate file at:

- **Unix/Linux/macOS**: `~/.doct/source.json`
- **Windows**: `%USERPROFILE%\.doct\source.json` (equivalent to `C:\Users\{username}\.doct\source.json`)
...
2. **Analytics**: Helps understand how users are discovering and installing Doct Code
...
- The `DoctLogger` class reads this file during telemetry initialization
...
```bash
cat ~/.doct/source.json
```

**Windows:**

```cmd
type %USERPROFILE%\.doct\source.json
```
...
./install-doct-with-source.bat -s github
```
...
- Ensure NVM is installed: `curl -o- https://doct-code-assets.oss-cn-hangzhou.aliyuncs.com/installation/install_nvm.sh | bash`
...
- The scripts require internet access to download Node.js and Doct Code
