@echo off
REM Script to install Node.js and Doct Code with source information
REM This script handles the installation process and sets the installation source
REM
REM Usage: install-doct-with-source.bat --source <source>
REM        install-doct-with-source.bat -s <source>
REM
...
echo ===========================================
echo Doct Code Installation Script with Source Tracking
echo ===========================================
echo.
...
if !ERRORLEVEL! EQU 0 (
    ...
    if !MAJOR_VERSION! GEQ 20 (
        ...
        if !ERRORLEVEL! NEQ 0 (
            echo ERROR: Failed to install Node.js. Cannot continue with Doct Code installation.
            exit /b 1
        )
    )
) else (
...
REM Verify npm is available before installing Doct Code
...
REM Install Doct Code with source information
echo INFO: Installing Doct Code with source: %SOURCE%
echo INFO: Running: %NPM_CMD% install -g @doct-code/doct-code@latest --registry https://registry.npmmirror.com
call "%NPM_CMD%" install -g @doct-code/doct-code@latest --registry https://registry.npmmirror.com

if %ERRORLEVEL% EQU 0 (
    echo SUCCESS: Doct Code installed successfully!
) else (
    echo ERROR: Failed to install Doct Code.
    exit /b 1
)

REM Create source.json only if --source or -s was explicitly provided
if not "!SOURCE!"=="unknown" (
    echo INFO: Creating source.json in %USERPROFILE%\.doct...

    set "DOCT_DIR=%USERPROFILE%\.doct"
    if not exist "!DOCT_DIR!" (
        mkdir "!DOCT_DIR!"
    )
    ...
    echo   "source": "!SOURCE!"
    echo }
    ) > "!DOCT_DIR!\source.json"

    echo SUCCESS: Installation source saved to %USERPROFILE%\.doct\source.json
)

REM Verify installation
call :CheckCommandExists doct
if %ERRORLEVEL% EQU 0 (
    echo SUCCESS: Doct Code is available as 'doct' command.
    call doct --version
    echo.
    echo INFO: Starting Doct Code...
    echo.
    call doct
) else (
    echo WARNING: Doct Code may not be in PATH. Please check your npm global bin directory.
    echo.
    echo ===========================================
    echo SUCCESS: Installation completed!
    echo The source information is stored in %USERPROFILE%\.doct\source.json
    echo.
    echo ===========================================
)
...
set "TEMP_DIR=%TEMP%\doct-nodejs-install"
