---
name: doct-code-claw
description: Use Doct Code as a Code Agent for code understanding, project generation, features, bug fixes, refactoring, and various programming tasks
---
...
```bash
npm install -g @doct-code/doct-code
```
...
```bash
doct auth coding-plan --region china --key $BAILIAN_CODING_PLAN_API_KEY
```

**If the environment variable does not exist**, interrupt and prompt the user to authenticate via `doct-oauth` or `coding-plan`:

```bash
doct auth
```

Or configure custom API after launching Doct Code via `/auth`.

## ACPX Integration

You should always use `acpx` to run Doct Code over the Agent Client Protocol (ACP) instead of scraping PTY sessions.
...
From now on, when you need to delegate work to Doct Code, use `acpx` instead of spawning raw terminal sessions. For example:

```bash
npx acpx@latest doct "fix the failing tests"
npx acpx@latest doct "refactor the auth module"
npx acpx@latest doct exec "one-shot: summarize this repo"
```
...
```bash
acpx doct 'inspect failing tests and propose a fix plan'
acpx doct 'apply the smallest safe fix and run tests'
```

### One-Shot Script Steps

```bash
acpx doct exec 'summarize repo purpose in 3 lines'
```

### Parallel Named Streams

```bash
acpx doct -s backend 'fix API pagination bug'
acpx doct -s docs 'draft changelog entry for release'
```

### Queue Follow-ups Without Waiting

```bash
acpx doct 'run full test suite and investigate failures'
acpx doct --no-wait 'after tests, summarize root causes and next steps'
```

### Machine-Readable Output for Orchestration

```bash
acpx --format json doct 'review current branch changes' > events.ndjson
```

### Repository-Wide Review with Permissive Mode

```bash
acpx --cwd ~/repos/my-project --approve-all doct -s pr-123 \
...
Full reference: https://raw.githubusercontent.com/DoctLM/doct-code/refs/heads/main/docs/users/features/commands.md

### Configuration

Config files (highest priority first): CLI args > env vars > system > project (`.doct/settings.json`) > user (`~/.doct/settings.json`) > defaults. Format: JSONC with env var interpolation.

Key settings:

| Setting                      | Description                               |
| ---------------------------- | ----------------------------------------- |
| `model.name`                 | Model to use (e.g. `doct-max`)            |
| `tools.approvalMode`         | `plan` / `default` / `auto_edit` / `yolo` |
| `permissions.allow/ask/deny` | Tool permission rules                     |
| `mcpServers.*`               | MCP server configurations                 |

Full reference: https://raw.githubusercontent.com/DoctLM/doct-code/refs/heads/main/docs/users/configuration/settings.md

### Authentication

Supports Doct OAuth (browser-based, 100 free requests/day, ending 2026-04-15 — switch to OpenRouter, Fireworks AI, or Alibaba Cloud ModelStudio after that) and OpenAI-compatible API keys.

Full reference: https://raw.githubusercontent.com/DoctLM/doct-code/refs/heads/main/docs/users/configuration/auth.md
...
Full reference: https://raw.githubusercontent.com/DoctLM/doct-code/refs/heads/main/docs/users/configuration/model-providers.md

### Key Features

| Feature       | Description                               | Docs                                                                                                    |
| ------------- | ----------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Approval Mode | Control tool execution permissions        | https://raw.githubusercontent.com/DoctLM/doct-code/refs/heads/main/docs/users/features/approval-mode.md |
| MCP           | Model Context Protocol server integration | https://raw.githubusercontent.com/DoctLM/doct-code/refs/heads/main/docs/users/features/mcp.md           |
| Skills        | Reusable skill system via `/skill`        | https://raw.githubusercontent.com/DoctLM/doct-code/refs/heads/main/docs/users/features/skills.md        |
| Sub-agents    | Delegate tasks to specialized agents      | https://raw.githubusercontent.com/DoctLM/doct-code/refs/heads/main/docs/users/features/sub-agents.md    |
| Sandbox       | Secure code execution environment         | https://raw.githubusercontent.com/DoctLM/doct-code/refs/heads/main/docs/users/features/sandbox.md       |
| Headless      | Non-interactive / CI mode                 | https://raw.githubusercontent.com/DoctLM/doct-code/refs/heads/main/docs/users/features/headless.md      |
