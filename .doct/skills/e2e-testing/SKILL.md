...
- **Reproducing bugs**: use the globally installed `doct` command — this matches
...
Run the CLI non-interactively with JSON output (`<doct>` = `doct` or
`node dist/cli.js` per above):

```bash
<doct> "your prompt here" \
  --approval-mode yolo \
  --output-format json \
  2>/dev/null
```
...
tmux new-session -d -s test -x 200 -y 50 \
  "cd /tmp/test-dir && <doct> --approval-mode yolo"
...
```bash
python3 .doct/skills/e2e-testing/scripts/token-stats.py 20  # last 20 requests
```
