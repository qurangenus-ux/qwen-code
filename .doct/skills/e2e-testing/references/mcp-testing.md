...
MCP servers are configured in `.doct/settings.json` under `mcpServers`. This is
the **only** location that works for E2E testing.

Common mistakes that waste time:

- `.mcp.json` — Claude Code convention, not Doct Code
...
- You're not running from the directory containing `.doct/settings.json`
