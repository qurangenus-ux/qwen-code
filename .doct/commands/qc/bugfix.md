...
Create `.doct/issues/` if it doesn't exist, then pipe the issue directly
...
Spawn the `test-engineer` agent and tell it to read `.doct/issues/issue-<number>.md`
...
Wait for the test engineer to finish. Then **read `.doct/issues/issue-<number>.md`**
...
`test-engineer` agent again and tell it to read `.doct/issues/issue-<number>.md`
